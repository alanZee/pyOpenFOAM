...
1. **Docker Desktop**: 需手动启动（已安装，daemon 未运行）
2. **8 个零物理求解器**: 需特定初始条件（PotentialFoam 需势场 BC、AcousticFoam 需声源等）
3. **可微分大网格**: 16x16 使用 3x 阻尼压力校正
4. **GPU 小网格**: kernel 启动开销导致 GPU 慢于 CPU
5. **CavitatingFoam**: 速度限制器 100 m/s，空化模型需进一步调参
6. **4 个求解器导入错误**: MagneticFoam/MhdFoam (张量类型不匹配)、SolidEquilibriumDisplacementFoam/StressFoam (未注册)
