"""
stitchMesh enhanced — enhanced mesh stitching with better coincident face
detection and non-conformal stitching support.

Extends :func:`stitch_mesh` with:

- **Spatial hashing**: O(1) face-centre lookup instead of brute-force.
- **Non-conformal stitching**: Supports faces with different vertex
  counts by matching face centres and projecting overlap areas.
- **Proximity-based matching**: Uses face-centre distance with
  configurable tolerance for robust matching even with mesh distortion.

Usage::

    from pyfoam.tools.stitch_mesh_enhanced import stitch_mesh_enhanced

    result = stitch_mesh_enhanced(
        mesh, "patch1", "patch2",
        tolerance=1e-4,
        non_conformal=True,
    )
...
__all__ = ["StitchEnhancedResult", "stitch_mesh_enhanced"]


@dataclass
class StitchEnhancedResult:
    """Result from :func:`stitch_mesh_enhanced`.
...
def stitch_mesh_enhanced(
    mesh: "FvMesh",
    patch1_name: str,
    patch2_name: str,
    tolerance: float = 1e-6,
    non_conformal: bool = False,
) -> StitchEnhancedResult:
    ...
    non_conformal : bool
        ...
        using face-centre proximity only.
    ...
    for fi1 in range(p1_start, p1_end):
        ...
        fc1 = pts1.mean(dim=0)
        key = _hash_point(fc1, tolerance)

        # Check this cell and neighbours in spatial hash
        best_fi2 = None
        best_dist = tolerance
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                for dz in (-1, 0, 1):
                    ...
                    for fi2, fc2 in p2_centres[nk]:
                        ...
                        if dist < best_dist:
                            if non_conformal or _faces_coincide(mesh, fi1, fi2, tolerance):
                                best_dist = dist
                                best_fi2 = fi2
    ...
    # Add stitched pairs as internal faces
