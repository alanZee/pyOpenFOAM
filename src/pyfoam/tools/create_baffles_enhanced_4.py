"""
createBaffles enhanced v4 — enhanced baffle creation with auto-thickness
detection and perforated baffle support (fourth generation).

Extends :func:`create_baffles_enhanced_3` with:

- **Auto-thickness detection**: Estimate local cell thickness and
  automatically size baffle gap for two-sided baffles.
- **Perforated baffles**: Model porous baffles with configurable
  porosity and pressure-drop coefficients.
- **Thermal resistance model**: Assign thermal resistance values
  to baffles for conjugate heat transfer setups.

Usage::

    from pyfoam.tools.create_baffles_enhanced_4 import create_baffles_enhanced_4

    result = create_baffles_enhanced_4(
        mesh,
        face_indices=[0, 1, 2],
        patch_name="baffle",
        dual_patches=True,
...
__all__ = ["BaffleEnhanced4Result", "create_baffles_enhanced_4"]


@dataclass
class BaffleEnhanced4Result:
    """Result from :func:`create_baffles_enhanced_4`.
    ...
    mean_thickness : float
        Mean auto-detected thickness at baffle locations.
    porosity : float
        Porosity value applied (0 = solid, 1 = fully open).
    pressure_drop_coefficient : float
        Darcy-Forchheimer pressure drop coefficient.
    thermal_resistance : float
        Assigned thermal resistance (K*m^2/W).
    """
...
def create_baffles_enhanced_4(
...
) -> BaffleEnhanced4Result:
    """Create baffles with auto-thickness, porosity, and thermal resistance.
    ...
    porosity : float
        Baffle porosity (0 = solid, 1 = fully open).
    pressure_drop_coefficient : float
        Darcy-Forchheimer coefficient for porous baffles.
    thermal_resistance : float
        Thermal resistance of the baffle (K*m^2/W).
    auto_thickness : bool
        If True, estimate local cell thickness at baffle locations.

    Returns
    -------
    BaffleEnhanced4Result
        Mesh with baffles and physics parameters.
    """
    from pyfoam.mesh.fv_mesh import FvMesh
...
def _detect_local_thickness(mesh, zone_specs, n_internal):
    """Estimate local cell thickness at baffle face locations.

    Uses the distance between the cell centres on either side of
    the internal face as a thickness proxy.
    """
    try:
    ...
    for _, fs, _ in zone_specs:
        for fi in fs:
            if fi < n_internal:
                own = int(mesh.owner[fi].item())
                nbr = int(mesh.neighbour[fi].item())
                d = float(((cc[own] - cc[nbr]) ** 2).sum() ** 0.5)
