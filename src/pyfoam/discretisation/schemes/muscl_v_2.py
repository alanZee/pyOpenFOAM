"""
MUSCLV2 interpolation scheme — v2 vector variant with van Albada limiter.

Improves on MUSCLV by using the van Albada limiter instead of minmod,
providing smoother transitions and better accuracy in smooth regions:
...
__all__ = ["MUSCLV2Interpolation"]


class MUSCLV2Interpolation(InterpolationScheme):
    """v2 vector variant of MUSCL TVD interpolation.

    Uses the van Albada limiter for smoother transitions than the minmod
    limiter used in MUSCLV, applied independently to each vector component.
    ...
    def interpolate(
        ...
        """MUSCLV2 interpolation of cell vector values to faces.
        ...
        if phi.dim() != 2 or phi.shape[1] != 3:
            raise ValueError(
                ...
                f"MUSCLV2 operates on vector fields."
            )
        if face_flux is None:
            raise ValueError(
                "MUSCLV2Interpolation requires 'face_flux'."
            )
        ...
        if mesh.n_cells <= 2:
            ...
            # v2 改进：van Albada 限制器: ψ(r) = (r^2 + r) / (1 + r^2)
            r2 = r * r
            psi = (r2 + r) / (1.0 + r2)
