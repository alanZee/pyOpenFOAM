"""
LinearUpwindFit2 interpolation scheme — v2 variant with distance-squared weighting.

Improves on linearUpwindFit by using distance-squared weighting in the
least-squares gradient reconstruction, reducing sensitivity to irregular
cell spacing:

    φ_f = φ_up + (∇φ)_fit2 · (r_f - r_up)

where (∇φ)_fit2 uses 1/d^2 weights instead of 1/d.

OpenFOAM syntax::

    divSchemes
    {
        default    Gauss linearUpwindFit2;
    }
"""
...
__all__ = ["LinearUpwindFit2Interpolation"]


class LinearUpwindFit2Interpolation(InterpolationScheme):
    """Linear upwind interpolation with distance-squared weighted least-squares fit.

    Uses 1/d^2 weighting instead of 1/d for gradient reconstruction,
    providing better accuracy on meshes with irregular cell spacing.
    ...
    def _compute_cell_gradients(self, phi: torch.Tensor) -> torch.Tensor:
        """Compute cell-centre gradients using distance-squared weighted fit.

        Uses 1/d^2 weighting for improved accuracy on non-uniform meshes.
    ...
    def interpolate(
        ...
        """LinearUpwindFit2 interpolation of cell values to faces.
        ...
        if face_flux is None:
            raise ValueError(
                "LinearUpwindFit2Interpolation requires 'face_flux'."
            )
