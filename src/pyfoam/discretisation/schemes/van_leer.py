"""
Van Leer interpolation scheme — TVD with van Leer limiter.

A second-order TVD scheme using the van Leer flux limiter:

    φ_f = φ_up + ψ(r) * (φ_f_linear - φ_up)

where ψ(r) = (r + |r|) / (1 + |r|) and r is the ratio of consecutive
(upstream-to-upstream vs. upstream-to-downstream) gradients.

The van Leer limiter smoothly transitions between first-order upwind
(ψ=0 in regions of steep gradients) and second-order linear (ψ=1 in
smooth regions).
"""
...
__all__ = ["VanLeerInterpolation"]


class VanLeerInterpolation(InterpolationScheme):
    """Van Leer TVD interpolation scheme.

    Applies the van Leer flux limiter to linear interpolation to prevent
    ...
    .. math::
        ...
        \\phi_f = \\phi_{up} + \\psi(r) (\\phi_f^{linear} - \\phi_{up})

    with :math:`\\psi(r) = (r + |r|) / (1 + |r|)`.
    ...
    def interpolate(
        ...
        """Van Leer TVD interpolation of cell values to faces.
        ...
        if face_flux is None:
            raise ValueError(
                "VanLeerInterpolation requires 'face_flux'."
            )
