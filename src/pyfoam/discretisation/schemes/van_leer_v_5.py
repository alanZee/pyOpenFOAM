"""
VanLeerV5 interpolation scheme — v5 vector variant with SMART limiter.

v5 改进：使用 SMART (Sharp and Monotonic Algorithm for Realistic Transport)
限制器替代 v4 的 Koren 限制器。SMART 提供更锐利的分辨率同时保持
单调性：

    ψ(r) = max(0, min(4r, (3+4r)/4, 1))

OpenFOAM syntax::

    divSchemes
    {
        default    Gauss vanLeerV5;
    }
"""
...
__all__ = ["VanLeerV5Interpolation"]


class VanLeerV5Interpolation(InterpolationScheme):
    """v5 vector variant of Van Leer TVD interpolation.

    Uses the SMART limiter for sharper resolution within the TVD region,
    ...
    def interpolate(
        ...
        """VanLeerV5 interpolation of cell vector values to faces."""
        if phi.dim() != 2 or phi.shape[1] != 3:
            raise ValueError(
                f"Expected (n_cells, 3) input tensor, got shape {phi.shape}. "
                f"VanLeerV5 operates on vector fields."
            )
        if face_flux is None:
            raise ValueError(
                "VanLeerV5Interpolation requires 'face_flux'."
            )
        ...
        if mesh.n_cells <= 2:
            ...
            # v5: SMART 限制器: ψ(r) = max(0, min(4r, (3+4r)/4, 1))
            psi = torch.max(
                torch.zeros_like(r),
                torch.min(
                    4.0 * r,
