"""
GammaV4 interpolation scheme — v4 vector variant with hyperbolic tangent blending.

v4 改进：使用双曲正切函数替代 v3 的 sigmoid 进行 Peclet 混合，
提供更陡峭的过渡特性，在高 Peclet 数时更快地切换到纯上风格式：

    Γ = 0.5 * (1 + tanh(k * (|Pe| - Pe_threshold)))

OpenFOAM syntax::

    divSchemes
    {
        default    Gauss gammaV4;
    }
"""
...
__all__ = ["GammaV4Interpolation"]


class GammaV4Interpolation(InterpolationScheme):
    """v4 vector variant of Gamma interpolation with tanh blending.

    Uses a hyperbolic tangent function for Peclet-number-based blending
    between upwind and linear interpolation.
    ...
    steepness : float
        Steepness of the tanh transition.  Default is 3.0.
    pe_threshold : float
        Peclet number threshold for the tanh midpoint.  Default is 1.0.
    """

    def __init__(
        self, mesh, steepness: float = 3.0, pe_threshold: float = 1.0,
    ) -> None:
        super().__init__(mesh)
    ...
    def interpolate(
        ...
        """GammaV4 interpolation of cell vector values to faces.
        ...
        if phi.dim() != 2 or phi.shape[1] != 3:
            raise ValueError(
                ...
                f"GammaV4 operates on vector fields."
            )
        if face_flux is None:
            raise ValueError(
                "GammaV4Interpolation requires 'face_flux'."
            )
