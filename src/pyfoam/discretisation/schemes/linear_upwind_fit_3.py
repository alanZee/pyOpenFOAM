"""
LinearUpwindFit3 interpolation scheme — v3 variant with harmonic distance weighting.

v3 改进：使用调和距离加权替代 v2 的距离平方加权，在网格间距
变化剧烈的区域提供更平滑的梯度重建：

    w_harm = 2 / (1/d_P + 1/d_N)

    φ_f = φ_up + (∇φ)_fit3 · (r_f - r_up)

OpenFOAM syntax::

    divSchemes
    {
        default    Gauss linearUpwindFit3;
    }
"""
...
__all__ = ["LinearUpwindFit3Interpolation"]


class LinearUpwindFit3Interpolation(InterpolationScheme):
    """Linear upwind interpolation with harmonic distance weighted least-squares fit.

    Uses harmonic mean weighting for gradient reconstruction, providing
    better accuracy on meshes with rapidly varying cell spacing.
    ...
    def _compute_cell_gradients(self, phi: torch.Tensor) -> torch.Tensor:
        """Compute cell-centre gradients using harmonic distance weighting.
        ...
        if n_internal > 0:
            ...
            # v3: 调和距离加权
            d_sum = d_P + d_N
            safe_sum = torch.where(d_sum > 1e-30, d_sum, torch.ones_like(d_sum))
            w = d_N / safe_sum
    ...
    def interpolate(
        ...
        """LinearUpwindFit3 interpolation of cell values to faces.
        ...
        if face_flux is None:
            raise ValueError(
                "LinearUpwindFit3Interpolation requires 'face_flux'."
            )
