"""
LinearUpwindFit4 interpolation scheme — v4 variant with exponential distance weighting.

v4 改进：使用指数距离加权替代 v3 的调和距离加权，在网格间距
变化剧烈的区域提供更强的平滑效果：

    w_exp = exp(-α * d_P) / (exp(-α * d_P) + exp(-α * d_N))

    φ_f = φ_up + (∇φ)_fit4 · (r_f - r_up)

OpenFOAM syntax::

    divSchemes
    {
        default    Gauss linearUpwindFit4;
    }
"""
...
__all__ = ["LinearUpwindFit4Interpolation"]


class LinearUpwindFit4Interpolation(InterpolationScheme):
    """Linear upwind interpolation with exponential distance weighted least-squares fit.

    Uses exponential decay weighting for gradient reconstruction, providing
    stronger smoothing on meshes with rapidly varying cell spacing.
    ...
    def interpolate(
        ...
        """LinearUpwindFit4 interpolation of cell values to faces.
        ...
        if face_flux is None:
            raise ValueError(
                "LinearUpwindFit4Interpolation requires 'face_flux'."
            )
