"""
CubicUpwindFit3 interpolation scheme — v3 variant with enhanced blending.

v3 改进：使用自适应混合系数替代 v2 的固定距离平方混合，混合系数
根据局部网格非正交度动态调整：
...
__all__ = ["CubicUpwindFit3Interpolation"]


class CubicUpwindFit3Interpolation(InterpolationScheme):
    """Cubic upwind interpolation with adaptive blending based on mesh quality.
    ...
    def _compute_cell_gradients(self, phi: torch.Tensor) -> torch.Tensor:
        """Compute cell-centre gradients using distance-weighted fit.
    ...
    def interpolate(
        ...
        """CubicUpwindFit3 interpolation of cell values to faces.
        ...
        if face_flux is None:
            raise ValueError(
                "CubicUpwindFit3Interpolation requires 'face_flux'."
            )
        ...
        if mesh.n_cells < 3:
            ...
            d_hat = d_vec / d_mag
            n_hat = mesh.face_areas[:n_internal] / mesh.face_areas[:n_internal].norm(
                dim=1, keepdim=True
            ).clamp(min=1e-30)
            cos_theta = (d_hat * n_hat).sum(dim=1).abs()
            # 非正交度越大，越依赖线性插值
            alpha = (1.0 / (1.0 + cos_theta)).unsqueeze(-1)

            # 混合线性插值和三次外推
            face_values[:n_internal] = (
                (1.0 - alpha.squeeze(-1)) * phi_linear
                + alpha.squeeze(-1) * phi_extrap
            )
