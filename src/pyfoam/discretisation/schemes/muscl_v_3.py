"""
MUSCLV3 interpolation scheme — v3 vector variant with Sweby limiter.

v3 改进：使用 Sweby 双参数限制器替代 v2 的 van Albada 限制器。
Sweby 限制器通过参数 β 在 minmod (β=0) 和 superbee (β=1) 之间
提供可调节的限制行为：

    ψ(r) = max(0, min(β*r, 1), min(r, β))   for β ∈ [1, 2]

OpenFOAM syntax::

    divSchemes
    {
        default    Gauss MUSCLV3;
    }
"""
...
__all__ = ["MUSCLV3Interpolation"]


class MUSCLV3Interpolation(InterpolationScheme):
    """v3 vector variant of MUSCL TVD interpolation.

    Uses the Sweby two-parameter limiter with adjustable beta for
    flexible monotonicity control, applied independently per component.
    ...
    beta : float
        Sweby parameter in [1, 2].  1.0 = minmod, 2.0 = superbee.
        Default is 1.5.
    """

    def __init__(self, mesh, beta: float = 1.5) -> None:
        super().__init__(mesh)
        self._beta = beta
    ...
    def interpolate(
        ...
        """MUSCLV3 interpolation of cell vector values to faces.
        ...
        if phi.dim() != 2 or phi.shape[1] != 3:
            raise ValueError(
                ...
                f"MUSCLV3 operates on vector fields."
            )
        if face_flux is None:
            raise ValueError(
                "MUSCLV3Interpolation requires 'face_flux'."
            )
        ...
        if mesh.n_cells <= 2:
            ...
            # v3: Sweby 限制器: ψ(r) = max(0, min(β*r, 1), min(r, β))
            beta = self._beta
            one = torch.ones_like(r)
            beta_t = torch.full_like(r, beta)
            psi = torch.max(
                torch.zeros_like(r),
                torch.max(
                    torch.min(beta * r, one),
                    torch.min(r, beta_t),
                ),
            )
