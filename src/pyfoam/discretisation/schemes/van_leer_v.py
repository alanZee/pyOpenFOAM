"""
VanLeerV interpolation scheme — vector variant of Van Leer TVD.

Applies the Van Leer TVD limiter independently to each component of
a vector field:

    φ_f,i = φ_up,i + ψ(r_i) * (φ_f,i^linear - φ_up,i)

where ψ(r) = (r + |r|) / (1 + |r|) is the Van Leer limiter applied
per component.
...
__all__ = ["VanLeerVInterpolation"]


class VanLeerVInterpolation(InterpolationScheme):
    """Vector variant of Van Leer TVD interpolation.

    Applies the Van Leer flux limiter independently to each component
    of a vector field to prevent overshoots while preserving second-order
    ...
    def interpolate(
        ...
        """VanLeerV interpolation of cell vector values to faces.
        ...
        if phi.dim() != 2 or phi.shape[1] != 3:
            raise ValueError(
                ...
                f"VanLeerV operates on vector fields."
            )
        if face_flux is None:
            raise ValueError(
                "VanLeerVInterpolation requires 'face_flux'."
            )
        ...
        if mesh.n_cells <= 2:
            ...
            d_up = torch.where(mask, fc - cc_P, fc - cc_N)    # (n_int, 3)
            grad_up = torch.where(
                mask.unsqueeze(-1),                             # (n_int, 1, 3)
                grad_phi[idx_own],
                grad_phi[idx_nei],
            )  # (n_int, 3, 3)

            # grad_up[:, comp, :] · d_up -> (n_int, 3)
