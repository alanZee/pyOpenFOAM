"""
Enhanced oxidation models v7.

- :class:`PowerLawOxidation` -- power-law oxidation
- :class:`NthOrderOxidation` -- n-th order oxidation
"""
...
__all__ = ["PowerLawOxidation", "NthOrderOxidation"]


class PowerLawOxidation(OxidationModel):
    """power-law oxidation."""
...
class NthOrderOxidation(OxidationModel):
    """n-th order oxidation."""
    def __init__(self, A=1.0, E_a=8e4, **kw): self.A = A; self.E_a = E_a
    def oxidise(self, dt, diameter, temperature, oxygen_mass_fraction=0.23, fluid_density=1.0):
        if diameter < 1e-15 or temperature < 1.0: return 0.0
        k = self.A * math.exp(-self.E_a / (8.314 * max(temperature, 1.0)))
