"""
Enhanced reacting models models v6.

- :class:`KineticReacting` -- kinetic rate reacting
- :class:`DiffusionReacting` -- diffusion controlled reacting
"""
...
__all__ = ["KineticReacting", "DiffusionReacting"]


class KineticReacting(ReactingModel):
    """kinetic rate reacting."""
...
class DiffusionReacting(ReactingModel):
    """diffusion controlled reacting."""
    def __init__(self, A=1e3, E_a=8e4, **kw): self.A = A; self.E_a = E_a
    def react(self, dt, diameter, temperature, fluid_temperature, species_mass_fraction=1.0):
        if diameter < 1e-15 or temperature < 1.0: return {"diameter": diameter, "mass_loss": 0.0, "heat_release": 0.0}
        k = self.A * math.exp(-self.E_a / (8.314 * max(temperature, 1.0)))
