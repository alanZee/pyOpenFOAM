"""
Enhanced reacting models models v9.

- :class:`GlobalReactionModel` -- global reaction model
- :class:`DetailedReactionModel` -- detailed mechanism
"""
...
__all__ = ["GlobalReactionModel", "DetailedReactionModel"]


class GlobalReactionModel(ReactingModel):
...
class DetailedReactionModel(ReactingModel):
    """detailed mechanism."""
    def __init__(self, A=1e3, E_a=8e4, **kw): self.A = A; self.E_a = E_a
    def react(self, dt, diameter, temperature, fluid_temperature, species_mass_fraction=1.0):
        if diameter < 1e-15 or temperature < 1.0: return {"diameter": diameter, "mass_loss": 0.0, "heat_release": 0.0}
