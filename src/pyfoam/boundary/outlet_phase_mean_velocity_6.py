"""
Enhanced outlet phase mean velocity boundary condition (v6).

Extends ``outletPhaseMeanVelocity5`` with a wall-distance velocity correction
and enhanced mass conservation using face-area weighting::

    alpha_safe = max(alpha, alphaMin)
    // Turbulent Prandtl correction (from v5)
    // TKE coupling (from v5)
    // Pressure-relaxation correction (from v5)
    // Wall-distance velocity correction
    y_plus = u_tau * y / nu
    f_wall = 1 - exp(-y_plus / wallDistCoeff)
    U_wall = U_outlet * (wallCoeff + (1 - wallCoeff) * f_wall)
    // Area-weighted mass conservation
    m_dot_face = rho * alpha * U * n * A
    m_dot_target_face = m_dot_target * A / A_total
    U *= m_dot_target_face / (m_dot_face + 1e-30)
    U = clamp(|U|, 0, Umax) * direction

In OpenFOAM syntax::

    type              outletPhaseMeanVelocity6;
    Umean             uniform (1 0 0);
...
__all__ = ["OutletPhaseMeanVelocity6BC"]


@BoundaryCondition.register("outletPhaseMeanVelocity6")
class OutletPhaseMeanVelocity6BC(BoundaryCondition):
    """Enhanced outlet phase mean velocity BC v6 with wall-distance correction.
    ...
    def apply(
        ...
        """Apply v6 enhanced outlet phase-mean velocity BC with wall-distance correction.
        ...
        # Area-weighted mass conservation correction
    ...
    def matrix_contributions(
        ...
        """Penalty method for v6 outlet phase mean velocity BC."""
        device = get_device()
