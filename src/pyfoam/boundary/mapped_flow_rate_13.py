"""Enhanced mapped flow rate boundary condition (v13).

In OpenFOAM syntax::

    type        mappedFlowRate13;
    value       uniform 0;

Coefficients:
    - Standard mapped flow rate parameters (from base and earlier versions).
    - ``axial_decay_coeff`` (float): Axial velocity decay correction. (default 0.3).
"""
...
__all__ = ["MappedFlowRate13BC"]


@BoundaryCondition.register("mappedFlowRate13")
class MappedFlowRate13BC(BoundaryCondition):
    """Enhanced mapped flow rate v13.
    ...
    def apply(
        ...
        """Apply enhanced mapped flow rate v13."""
        device = field.device
        dtype = field.dtype
        ...
        # v13 enhancement: axial velocity decay correction.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v13 enhanced mapped flow rate BC."""
        device = get_device()
