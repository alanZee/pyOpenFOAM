"""
Enhanced pressure wave transmissive boundary condition (v5).

Extends ``pressureWaveTransmissive4`` with improved NSCBC treatment that
adds a multi-scale relaxation approach and adaptive sigma::
    ...
    p_nscbc = p_wave + blending * (p_far - p_owner) * Ma
    f_damp = 1 / (1 + (f_cutoff / f_turb)^2)
    p_damp = damping * rho * k_turb * f_damp
    p_face = p_nscbc - p_damp

In OpenFOAM syntax::

    type        pressureWaveTransmissive5;
    phi         phi;
    rho         rho;
    psi         psi;
    gamma       1.4;
...
__all__ = ["PressureWaveTransmissive5BC"]


@BoundaryCondition.register("pressureWaveTransmissive5")
class PressureWaveTransmissive5BC(BoundaryCondition):
    """Enhanced pressure wave transmissive BC (v5) with improved NSCBC.
    ...
    def apply(
        ...
        """Apply v5 enhanced wave transmissive pressure BC.
    ...
    def matrix_contributions(
        ...
        """Relaxation-based matrix contribution for v5 wave transmissive."""
        device = get_device()
