"""
Enhanced mapped flow rate boundary condition (v9).

Extends ``mappedFlowRate8`` with wall-distance-weighted profile correction
and adaptive swirl damping based on local Reynolds number::

    rho_eff = rho * (1 - beta_thermal * (T - T_ref))
    n_eff = profileExponent * (1 + beta * log10(Re / ReRef))
    // Wall-distance-weighted profile (from v8 blending)
    y_plus = u_tau * y / nu
    w_wall = 1 - exp(-y_plus / wallDistCoeff)
    w_profile = w_turb * blendWeight + (1 - w_turb) * (1 - r/R)^n_eff
    w_final = w_wall * w_profile + (1 - w_wall) * wallWeight
    // Adaptive swirl damping
    Re_local = |U| * D_h / nu
    swirl_damp = 1 / (1 + (Re_local / swirlReRef)^swirlDampExp)
    swirl_local(r) = swirlRatio * swirl_damp * (R / r)^swirlExponent * exp(-decayCoeff * r / R)
    // Time-averaged correction (from v8)
    m_dot_avg = tauAvg * m_dot_prev + (1 - tauAvg) * m_dot_current
    U *= m_dot_target / (m_dot_avg + 1e-30)

In OpenFOAM syntax::

    type              mappedFlowRate9;
...
__all__ = ["MappedFlowRate9BC"]


@BoundaryCondition.register("mappedFlowRate9")
class MappedFlowRate9BC(BoundaryCondition):
    """Enhanced mapped flow rate with wall-distance profile and adaptive swirl v9.
    ...
    def apply(
        ...
        """Set boundary-face velocity from v9 mapped mass flow rate with wall-distance profile.
        ...
        if n_eff > 0 and n > 1:
            ...
            if nu is not None and nu > 0:
                u_tau_est = 0.05 * u_mean_for_re  # rough estimate
        ...
        # Swirl velocity with adaptive Reynolds-number damping
        if self._swirl_ratio != 0 and n > 1:
            ...
            # Adaptive swirl damping based on local Reynolds number
            if nu is not None and nu > 0:
            ...
            swirl_local = self._swirl_ratio * swirl_damp * torch.exp(-self._decay_coeff * r_frac)
    ...
    def matrix_contributions(
        ...
        """Penalty method for v9 mapped flow rate BC."""
        device = get_device()
