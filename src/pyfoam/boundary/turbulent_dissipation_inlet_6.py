"""
Enhanced turbulent dissipation inlet boundary condition (v6).

Extends ``turbulentDissipationInlet5`` with a production-limiter correction
and a wall-normal-distance-aware damping term for improved near-wall behaviour::

    k = 1.5 * (I * |U|)^2
    y_plus = u_tau * y / nu
    // Log-law layer: eps = C_mu^0.75 * k^1.5 / (kappa * y)
    // Buffer layer: eps = 2 * nu * k / y^2
    eps_base = blend(y_plus, yPlusLow, yPlusHigh) * eps_log + (1 - blend) * eps_buf
    // Production limiter: eps_prod = C_mu * k^2 / (nut + nu)
    eps_limited = max(eps_base, eps_prod * productionRatio)
    eps = clamp(eps_limited, epsilonMin, epsilonMax)

In OpenFOAM syntax::

    type        turbulentDissipationInlet6;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    intensity   0.05;
...
__all__ = ["TurbulentDissipationInlet6BC"]


@BoundaryCondition.register("turbulentDissipationInlet6")
class TurbulentDissipationInlet6BC(BoundaryCondition):
    """v6 enhanced turbulent dissipation inlet with production limiter.
    ...
    def apply(
        ...
        """Set boundary-face epsilon with production limiter and two-layer model.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v6 enhanced epsilon inlet BC."""
        device = get_device()
