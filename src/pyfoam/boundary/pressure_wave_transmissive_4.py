"""
Enhanced wave transmissive pressure boundary condition (v4).

Extends ``pressureWaveTransmissive3`` with frequency-dependent damping and
temperature-corrected speed of sound::
    ...
    p_wave = p_owner - rho * c_eff * (U_n - c_eff) * dp / denom
    f_damp = 1 / (1 + (f_cutoff / f_turb)^2)
    p_damp = damping * rho * k_turb * f_damp
    p_face = (1 - blending * Ma) * p_wave + blending * Ma * p_far - p_damp

In OpenFOAM syntax::

    type        pressureWaveTransmissive4;
    phi         phi;
    rho         rho;
    psi         psi;
    gamma       1.4;
...
__all__ = ["PressureWaveTransmissive4BC"]


@BoundaryCondition.register("pressureWaveTransmissive4")
class PressureWaveTransmissive4BC(BoundaryCondition):
    """Enhanced pressure wave transmissive BC (v4) with frequency-dependent damping.
    ...
    def apply(
        ...
        """Apply v4 enhanced wave transmissive pressure BC.
        ...
        # Speed of sound: use provided or compute from temperature
        if c is not None:
            c_val = c
        ...
        # Mach-dependent relaxation coefficient
        ...
        # Mach-dependent blending
        blend_eff = self._blending * (1.0 + sigma * ma)
        p_face = (1.0 - blend_eff) * p_wave + blend_eff * self._field_inf

        # Frequency-dependent turbulent fluctuation damping
        if k is not None:
            # Turbulent velocity scale -> characteristic frequency
            u_turb = torch.sqrt(torch.clamp(k, min=1e-30))
            f_turb = u_turb / (self._l_inf + 1e-30)
            # Low-pass filter: reduces damping at high frequencies
    ...
    def matrix_contributions(
        ...
        """Relaxation-based matrix contribution for v4 wave transmissive."""
        device = get_device()
        ...
        total_coeff = relax_coeff * (1.0 + self._sigma_base) + self._blending * area_mag
