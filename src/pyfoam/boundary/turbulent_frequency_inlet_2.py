"""
Enhanced turbulent frequency inlet boundary condition (v2).

Computes omega from turbulence intensity and length scale, combining
the intensity-based k estimation with the frequency formula::
...
__all__ = ["TurbulentFrequencyInlet2BC"]


@BoundaryCondition.register("turbulentFrequencyInlet2")
class TurbulentFrequencyInlet2BC(BoundaryCondition):
    """Enhanced turbulent frequency inlet with intensity and length scale.
    ...
    def apply(
        ...
        """Set boundary-face omega from intensity, velocity, and length scale.
        ...
        if k is not None:
            omega = torch.sqrt(k) / (self._C_mu ** 0.25 * self._length_scale + 1e-30)
        else:
    ...
    def matrix_contributions(
        ...
        """Penalty method for enhanced omega inlet BC."""
        device = get_device()
