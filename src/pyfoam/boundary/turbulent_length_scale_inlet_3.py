"""
Enhanced turbulent length scale inlet boundary condition (v3).

Extends ``turbulentLengthScaleInlet2`` with a hybrid blending between
computed and reference length scales plus a wall-distance-aware limiter::

    l_computed = C_mu^0.75 * k^1.5 / epsilon
    l_ref = lengthScaleFraction * D_h
    l_mix = alpha * l_computed + (1 - alpha) * l_ref
    l_mix = clamp(l_mix, lengthScaleMin, D_h * lengthScaleFraction)

In OpenFOAM syntax::

    type        turbulentLengthScaleInlet3;
    Cmu         0.09;
    intensity   0.05;
    lengthScale 0.01;
    lengthScaleMin  1e-6;
...
__all__ = ["TurbulentLengthScaleInlet3BC"]


@BoundaryCondition.register("turbulentLengthScaleInlet3")
class TurbulentLengthScaleInlet3BC(BoundaryCondition):
    """v3 enhanced turbulent length scale inlet with hybrid blending.
    ...
    Coefficients:
        ...
        - ``alpha``: Blending weight for computed l_mix (default 0.8).
    ...
    def alpha(self) -> float:
        """Blending weight for computed l_mix."""
        return self._alpha
    ...
    def apply(
        ...
        """Set boundary-face mixing length with hybrid blending and clamping.
        ...
        l_ref = l_max  # reference length = fraction of hydraulic diameter
        ...
        # Hybrid blending
        l_mix = self._alpha * l_computed + (1.0 - self._alpha) * l_ref
    ...
    def matrix_contributions(
        ...
        """Penalty method for v3 mixing length inlet BC."""
        device = get_device()
