"""
Enhanced turbulent dissipation inlet boundary condition (v8).

Extends ``turbulentDissipationInlet7`` with a wall-normal diffusion correction
and a Kolmogorov-scale limiter for very fine grids::
    ...
    eta = (nu^3 / eps_base)^0.25
    eps_kolm = nu^3 / (gridScale^4 + 1e-30)  (grid-dependent limit)
    eps = max(eps_base, eps_kolm)
    // Production limiter (carried from v7)
    S_ij = grad(U)
    P_k = 2 * nut * S_ij : S_ij
    eps_prod = C1 * P_k * k / (k + 1e-30)
    eps_limited = max(eps, eps_prod * productionRatio)
    eps = clamp(eps_limited, epsilonMin, epsilonMax)

In OpenFOAM syntax::

    type        turbulentDissipationInlet8;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    C1          1.44;
...
__all__ = ["TurbulentDissipationInlet8BC"]


@BoundaryCondition.register("turbulentDissipationInlet8")
class TurbulentDissipationInlet8BC(BoundaryCondition):
    """v8 enhanced turbulent dissipation inlet with Kolmogorov-scale limiter.
    ...
    def apply(
        ...
        """Set boundary-face epsilon with Kolmogorov-scale limiter.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v8 enhanced epsilon inlet BC."""
        device = get_device()
