"""
Enhanced turbulent kinetic energy inlet boundary condition (v5).

Extends ``turbulentKineticEnergyInlet4`` with a thermal-stratification
correction and a buoyancy-production term for mixed-convection inlets::

    k_intensity = 1.5 * (I * |U|)^2
    k_length = (epsilon * l_mix / C_mu^0.75)^(2/3)
    Ri = g * beta_thermal * dT * L / (U_ref^2)  (gradient Richardson number)
    Ri_eff = clamp(Ri, RiMin, RiMax)
    k_buoyancy = C_buoyancy * epsilon * Ri_eff
    k = alpha * k_intensity + (1 - alpha) * k_length + k_buoyancy
    k = clamp(k, kMin, kMax)

In OpenFOAM syntax::

    type        turbulentKineticEnergyInlet5;
    intensity   0.05;
    lengthScale 0.01;
    Cmu         0.09;
    alpha       0.8;
...
__all__ = ["TurbulentKineticEnergyInlet5BC"]


@BoundaryCondition.register("turbulentKineticEnergyInlet5")
class TurbulentKineticEnergyInlet5BC(BoundaryCondition):
    """v5 enhanced turbulent kinetic energy inlet with buoyancy production.
    ...
    def apply(
        ...
        """Set boundary-face k with buoyancy production and adaptive blending.
        ...
        if velocity is not None:
            ...
            if epsilon is not None:
                ...
                if self._Richardson != 0:
                    eps_for_buoy = epsilon if epsilon is not None else (
                        (self._C_mu ** 0.75) * (k_intensity ** 1.5) /
                        (self._length_scale + 1e-30)
                    )
                    k_buoyancy = self._C_buoyancy * eps_for_buoy * self._Richardson
                    k = k + torch.clamp(k_buoyancy, min=0.0)  # only add positive buoyancy
            else:
                k = k_intensity
    ...
    def matrix_contributions(
        ...
        """Penalty method for v5 enhanced k inlet BC."""
        device = get_device()
