"""
Enhanced mapped flow rate boundary condition (v7).

Extends ``mappedFlowRate6`` with swirl correction and angular momentum
conservation via a weighted blending approach::
    ...
    // Angular momentum conservation (from v6)
    L_target = swirlRatio * m_dot * R / (n_eff + 1)
    // Swirl correction with weighted blending
    omega_correction = swirlCorrection * L_target / (I_z + 1e-30)
    U_theta *= omega_correction
    U *= m_dot_target / m_dot_actual  (iterative correction)
    // Profile weight blending for improved radial distribution
    w_turb = 1 / (1 + (r / R * blendCoeff)^blendExponent)
    U *= w_turb * blendWeight + (1 - w_turb) * (1 - r/R)^n_eff

In OpenFOAM syntax::

    type              mappedFlowRate7;
...
__all__ = ["MappedFlowRate7BC"]


@BoundaryCondition.register("mappedFlowRate7")
class MappedFlowRate7BC(BoundaryCondition):
    """Enhanced mapped flow rate with swirl correction v7.
    ...
    def apply(
        ...
        """Set boundary-face velocity from v7 mapped mass flow rate with swirl correction.
        ...
        # Swirl velocity with angular momentum conservation and correction
        if self._swirl_ratio != 0 and n > 1:
            ...
            u_theta = (self._swirl_ratio * omega_correction *
    ...
    def matrix_contributions(
        ...
        """Penalty method for v7 mapped flow rate BC."""
        device = get_device()
