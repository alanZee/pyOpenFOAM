"""Enhanced turbulent intensity inlet boundary condition (v15).

In OpenFOAM syntax::

    type        turbulentIntensityInlet15;
    value       uniform 0;

Coefficients:
    - Standard turbulent intensity inlet parameters (from base and earlier versions).
    - ``realiz_coeff`` (float): Realizability constraint. (default 0.02).
"""
...
__all__ = ["TurbulentIntensityInlet15BC"]


@BoundaryCondition.register("turbulentIntensityInlet15")
class TurbulentIntensityInlet15BC(BoundaryCondition):
    """Enhanced turbulent intensity inlet v15.
    ...
    def __init__(self, patch: Patch, coeffs: dict[str, Any] | None = None) -> None:
        ...
        self._realiz_coeff = float(self._coeffs.get("realiz_coeff", 0.02))
    ...
    @property
    def realiz_coeff(self) -> float:
        return self._realiz_coeff
    def apply(
        self,
        field: torch.Tensor,
        patch_idx: int | None = None,
    ) -> torch.Tensor:
        """Apply enhanced turbulent intensity inlet v15."""
        device = field.device
        dtype = field.dtype
        ...
        # v15 enhancement: realizability constraint.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v15 enhanced turbulent intensity inlet BC."""
        device = get_device()
