"""
Enhanced pressure wave transmissive boundary condition (v8).

Extends ``pressureWaveTransmissive7`` with an entropy wave correction
and improved non-reflecting boundary treatment::
    ...
    s_local = Cp * ln(T_eff / T_ref) - R_specific * ln(p / p_ref)
    p_entropy = rho * c_eff * entropyCoeff * s_local
    // Improved NSCBC (from v7)
    L_plus = (c_eff / (2 * lInf)) * (p - p_ref) * (1 + Ma)
    L_minus = -(c_eff / (2 * lInf)) * (p - p_ref) * (1 - Ma)
    p_nscbc = p_owner + 0.5 * rho * lInf * (L_plus + L_minus) + p_entropy
    // Multi-scale turbulent damping (from v7)
    // Adaptive blending based on local Courant-like number
    p_face = (1 - blend_eff) * p_wave + blend_eff * p_nscbc - p_damp

In OpenFOAM syntax::

    type        pressureWaveTransmissive8;
    phi         phi;
    rho         rho;
    psi         psi;
    gamma       1.4;
...
__all__ = ["PressureWaveTransmissive8BC"]


@BoundaryCondition.register("pressureWaveTransmissive8")
class PressureWaveTransmissive8BC(BoundaryCondition):
    """Enhanced pressure wave transmissive BC (v8) with entropy wave correction.
    ...
    def apply(
        ...
        """Apply v8 enhanced wave transmissive pressure BC with entropy correction.
    ...
    def matrix_contributions(
        ...
        """Relaxation-based matrix contribution for v8 wave transmissive."""
        device = get_device()
