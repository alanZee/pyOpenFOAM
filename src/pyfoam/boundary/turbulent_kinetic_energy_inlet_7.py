"""
Enhanced turbulent kinetic energy inlet boundary condition (v7).

Extends ``turbulentKineticEnergyInlet6`` with compressibility correction
and a turbulent Mach number limiter::
    ...
    Ma_t = sqrt(2 * k) / c
    k_comp = k * (1 + alpha_comp * Ma_t^2)
    // Turbulent Mach limiter
    Ma_t_max = MaTMax
    k_limited = min(k_comp, 0.5 * rho * c^2 * Ma_t_max^2)
    k = alpha * k_intensity + (1 - alpha) * k_length + P_buoy
    k = min(k, k_limited)
    k = clamp(k, kMin, kMax)

In OpenFOAM syntax::

    type        turbulentKineticEnergyInlet7;
    intensity   0.05;
    lengthScale 0.01;
    Cmu         0.09;
    alpha       0.8;
...
__all__ = ["TurbulentKineticEnergyInlet7BC"]


@BoundaryCondition.register("turbulentKineticEnergyInlet7")
class TurbulentKineticEnergyInlet7BC(BoundaryCondition):
    """v7 enhanced turbulent kinetic energy inlet with compressibility correction.
    ...
    Coefficients:
        ...
        - ``alphaComp``: Compressibility correction coefficient (default 0.0).
        - ``MaTMax``: Maximum turbulent Mach number (default 0.5).
    ...
    def __init__(self, patch: Patch, coeffs: dict[str, Any] | None = None) -> None:
        ...
        self._alpha_comp = float(self._coeffs.get("alphaComp", 0.0))
        self._Ma_t_max = float(self._coeffs.get("MaTMax", 0.5))
    ...
    @property
    def alpha_comp(self) -> float:
        """Compressibility correction coefficient."""
        return self._alpha_comp

    @property
    def Ma_t_max(self) -> float:
        """Maximum turbulent Mach number."""
        return self._Ma_t_max
    ...
    def apply(
        ...
        """Set boundary-face k with compressibility correction and Ma_t limiter.
        ...
        if velocity is not None:
            ...
            # Compressibility correction
            if c is not None and self._alpha_comp != 0:
                Ma_t = torch.sqrt(2.0 * torch.clamp(k, min=1e-30)) / (c + 1e-30)
                k_comp = k * (1.0 + self._alpha_comp * Ma_t ** 2)
                # Turbulent Mach number limiter
                k_Ma_limited = 0.5 * 1.225 * c ** 2 * self._Ma_t_max ** 2
                k = torch.min(k_comp, torch.tensor(k_Ma_limited, dtype=dtype, device=device))
    ...
    def matrix_contributions(
        ...
        """Penalty method for v7 enhanced k inlet BC."""
        device = get_device()
