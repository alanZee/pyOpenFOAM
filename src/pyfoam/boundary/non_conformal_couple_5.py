"""Enhanced non-conformal couple boundary condition (v5).

In OpenFOAM syntax::

    type        nonConformalCouple5;
    value       uniform 0;

Coefficients:
    - Standard non-conformal couple parameters (from base and earlier versions).
    - ``smooth_coeff`` (float): Interface smoothing. (default 0.1).
"""
...
__all__ = ["NonConformalCouple5BC"]


@BoundaryCondition.register("nonConformalCouple5")
class NonConformalCouple5BC(BoundaryCondition):
    """Enhanced non-conformal couple v5.
    ...
    def apply(
        ...
        """Apply enhanced non-conformal couple v5."""
        device = field.device
        dtype = field.dtype
        ...
        # v5 enhancement: interface smoothing.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v5 enhanced non-conformal couple BC."""
        device = get_device()
