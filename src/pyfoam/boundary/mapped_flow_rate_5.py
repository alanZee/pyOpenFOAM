"""
Enhanced mapped flow rate boundary condition (v5).

Extends ``mappedFlowRate4`` with swirl-corrected radial redistribution
and a thermally-expanded density model that accounts for variable specific
heat::

    rho_eff = rho * (1 - beta_thermal * (T - T_ref))
    Cp_eff = Cp * (1 + gamma_Cp * (T - T_ref))
    q_thermal = m_dot * Cp_eff * dT / (rho_eff * A)
...
__all__ = ["MappedFlowRate5BC"]


@BoundaryCondition.register("mappedFlowRate5")
class MappedFlowRate5BC(BoundaryCondition):
    """Enhanced mapped flow rate with swirl correction and thermal expansion v5.
    ...
    def apply(
        ...
        """Set boundary-face velocity from v5 adaptive mapped mass flow rate.
        ...
        # Swirl velocity component with configurable radial exponent
        if self._swirl_ratio != 0 and n > 1:
            ...
            # Swirl with configurable radial exponent
            u_theta = self._swirl_ratio * velocity_mag * (r_frac ** self._swirl_exponent)
    ...
    def matrix_contributions(
        ...
        """Penalty method for v5 adaptive mapped flow rate BC."""
        device = get_device()
