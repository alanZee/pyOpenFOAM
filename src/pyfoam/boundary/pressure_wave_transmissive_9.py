"""
Enhanced pressure wave transmissive boundary condition (v9).

Extends ``pressureWaveTransmissive8`` with a viscous damping correction
and improved entropy-wave non-reflecting treatment::
    ...
    Re_tau = rho * |U| * lInf / mu
    visc_damp = viscCoeff * mu * |U| / (lInf * (1 + Re_tau / viscReRef))
    // Entropy wave correction (from v8)
    // NSCBC (from v8)
    p_face = (1 - blend_eff) * p_wave + blend_eff * p_nscbc - p_damp - visc_damp

In OpenFOAM syntax::

    type        pressureWaveTransmissive9;
    phi         phi;
    rho         rho;
    psi         psi;
    gamma       1.4;
...
__all__ = ["PressureWaveTransmissive9BC"]


@BoundaryCondition.register("pressureWaveTransmissive9")
class PressureWaveTransmissive9BC(BoundaryCondition):
    """Enhanced pressure wave transmissive BC (v9) with viscous damping correction.
    ...
    def apply(
        ...
        """Apply v9 enhanced wave transmissive pressure BC with viscous damping.
    ...
    def matrix_contributions(
        ...
        """Relaxation-based matrix contribution for v9 wave transmissive."""
        device = get_device()
        ...
        total_coeff = relax_coeff * (1.0 + self._sigma_base + self._nscbc_sigma + self._visc_coeff) + self._blending * area_mag
