"""
Enhanced pressure wave transmissive boundary condition (v6).

Extends ``pressureWaveTransmissive5`` with improved NSCBC treatment::

    T_eff = T_ref + gamma * (p - p_ref) / (rho * Cp)
    c_eff = sqrt(gamma * R_specific * T_eff)
    Ma = |U| / c_eff
    sigma = sigma_base * (1 + gamma * Ma^2) * (1 + beta_sigma * log(1 + Re_t))
    // Improved NSCBC with multi-wave decomposition
    L_plus = (c_eff / (2 * lInf)) * (p - p_ref) * (1 + Ma)
    L_minus = -(c_eff / (2 * lInf)) * (p - p_ref) * (1 - Ma)
    p_face = p_owner + 0.5 * rho * lInf * (L_plus + L_minus)
...
In OpenFOAM syntax::
    ...
    type        pressureWaveTransmissive6;
    phi         phi;
    rho         rho;
    psi         psi;
    gamma       1.4;
...
__all__ = ["PressureWaveTransmissive6BC"]


@BoundaryCondition.register("pressureWaveTransmissive6")
class PressureWaveTransmissive6BC(BoundaryCondition):
    """Enhanced pressure wave transmissive BC (v6) with improved NSCBC.
    ...
    def apply(
        ...
        """Apply v6 enhanced wave transmissive pressure BC with improved NSCBC.
        ...
        # Blend between wave transmissive and NSCBC
    ...
    def matrix_contributions(
        ...
        """Relaxation-based matrix contribution for v6 wave transmissive."""
        device = get_device()
