"""
Enhanced wall lubrication force BC — Tomiyama model.

Implements the Tomiyama et al. (2002) wall lubrication force model for
bubble flows.  The model provides an enhanced wall lubrication formulation
that accounts for the Eötvös number dependence of the force coefficient::

    F_wl = C_w * rho_c * alpha_d * |V_slip|^2 / D_p * n_w

The Tomiyama wall lubrication coefficient is Eötvös-dependent::

    C_w(Eo) = f(Eo) / D_p * C_w0

where:
    - ``C_w0`` is a base coefficient (default: 0.05)
    - ``Eo`` is the Eötvös number: Eo = g * |rho_c - rho_d| * d^2 / sigma
    - ``f(Eo)`` is a piecewise-continuous correlation

The correlation for f(Eo)::

    Eo < 1.0:        f = 0.0
    1 <= Eo < 5:     f = 0.474 * (1 - exp(-0.0183 * Eo)) * exp(1.48 * Eo)
    5 <= Eo < 33:    f = 0.0219 * Eo
    Eo >= 33:        f = 0.474

In OpenFOAM syntax::

    type              tomiyamaWallLubrication;
    alpha             alpha.water;
    Cw                0.05;        // base wall lubrication coefficient
    Dp                0.003;       // bubble diameter (m)
    rhoContinuous     rho.liquid;  // continuous phase density field
    rhoDispersed      rho.air;     // dispersed phase density field
...
__all__ = ["TomiyamaWallLubricationBC"]


@BoundaryCondition.register("tomiyamaWallLubrication")
class TomiyamaWallLubricationBC(BoundaryCondition):
    """Tomiyama wall lubrication force BC for bubble flows.

    Applies the Tomiyama et al. (2002) wall lubrication model with
    Eötvös-number-dependent coefficient.  The force pushes the
    dispersed phase away from walls and is especially suited for
    air-water bubble columns and bubble-driven flows.
    ...
    Coefficients:
        ...
        - ``rho_c``: Continuous-phase density in kg/m³ (default: 1000.0).
        - ``rho_d``: Dispersed-phase density in kg/m³ (default: 1.225).
    ...
    def eotvos_number(self) -> float:
        """Compute Eötvös number: Eo = g * |rho_c - rho_d| * d^2 / sigma."""
        g = 9.81
        return g * abs(self._rho_c - self._rho_d) * self._Dp ** 2 / self._sigma

    def f_eotvos(self, Eo: float) -> float:
        """Tomiyama's Eötvös-dependent wall lubrication correlation.
        ...
        -------
        float
            f(Eo) value for the wall lubrication coefficient.
        """
        if Eo < 1.0:
            return 0.0
        elif Eo < 5.0:
    ...
    def effective_coefficient(self, wall_distance: torch.Tensor) -> torch.Tensor:
        """Compute the Tomiyama wall lubrication coefficient.
        ...
        return torch.full_like(wall_distance, self._Cw0 * f_eo / self._Dp)
    ...
    def apply(
        ...
        """Apply Tomiyama wall lubrication velocity correction.
    ...
    def matrix_contributions(
        ...
        """Tomiyama wall lubrication force source contribution.

        Adds a Eötvös-dependent source term for wall-adjacent cells::
        ...
        # Tomiyama Eo-dependent coefficient
        ...
        # Density (use continuous-phase density for Tomiyama model)
        rho_val = self._rho_c

        # Force coefficient: Cw_eff * rho_c * alpha * area / Dp
