"""
Enhanced turbulent frequency inlet boundary condition (v7).

Extends ``turbulentFrequencyInlet6`` with an adaptive production limiter
and a strain-rate-dependent frequency estimate::
    ...
    // Strain-rate-based production correction
    S_ij = grad(U)  (mean strain rate magnitude)
    nut = k / (betaStar * omega + 1e-30)
    P_k = 2 * nut * S_ij^2
    omega_prod = P_k / (beta1 * k + 1e-30)
    omega_limited = max(omega_base, omega_prod * productionRatio)
    omega = clamp(omega_limited, omegaMin, omegaMax)

In OpenFOAM syntax::

    type        turbulentFrequencyInlet7;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    beta1       0.075;
...
__all__ = ["TurbulentFrequencyInlet7BC"]


@BoundaryCondition.register("turbulentFrequencyInlet7")
class TurbulentFrequencyInlet7BC(BoundaryCondition):
    """v7 enhanced turbulent frequency inlet with strain-rate production limiter.
    ...
    def apply(
        ...
        """Set boundary-face omega with strain-rate production limiter.
        ...
        if k is not None and nu is not None and nu > 0:
            ...
            if strain_rate is not None:
                nut_est = k / (self._beta_star * omega_base + 1e-30)
    ...
    def matrix_contributions(
        ...
        """Penalty method for v7 enhanced omega inlet BC."""
        device = get_device()
