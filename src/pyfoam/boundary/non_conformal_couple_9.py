"""Enhanced non-conformal couple boundary condition (v9).

In OpenFOAM syntax::

    type        nonConformalCouple9;
    value       uniform 0;

Coefficients:
    - Standard non-conformal couple parameters (from base and earlier versions).
    - ``anisotropy_coeff`` (float): Anisotropic interpolation correction. (default 0.05).
"""
...
__all__ = ["NonConformalCouple9BC"]


@BoundaryCondition.register("nonConformalCouple9")
class NonConformalCouple9BC(BoundaryCondition):
    """Enhanced non-conformal couple v9.
    ...
    def apply(
        ...
        """Apply enhanced non-conformal couple v9."""
        device = field.device
        dtype = field.dtype
        ...
        # v9 enhancement: anisotropic interpolation correction.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v9 enhanced non-conformal couple BC."""
        device = get_device()
