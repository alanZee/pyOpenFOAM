"""Enhanced turbulent length scale inlet boundary condition (v13).

In OpenFOAM syntax::

    type        turbulentLengthScaleInlet13;
    value       uniform 0;

Coefficients:
    - Standard turbulent length scale inlet parameters (from base and earlier versions).
    - ``curvature_coeff`` (float): Curvature correction for length scale. (default 0.03).
"""
...
__all__ = ["TurbulentLengthScaleInlet13BC"]


@BoundaryCondition.register("turbulentLengthScaleInlet13")
class TurbulentLengthScaleInlet13BC(BoundaryCondition):
    """Enhanced turbulent length scale inlet v13.
    ...
    def apply(
        ...
        """Apply enhanced turbulent length scale inlet v13."""
        device = field.device
        dtype = field.dtype
        ...
        # v13 enhancement: curvature correction for length scale.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v13 enhanced turbulent length scale inlet BC."""
        device = get_device()
