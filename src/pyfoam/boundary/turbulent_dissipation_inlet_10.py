"""
Enhanced turbulent dissipation inlet boundary condition (v10).

Extends ``turbulentDissipationInlet9`` with a dynamic time-scale limiter
and a vortex-stretching correction::
    ...
    tau_t = k / (eps + 1e-30)
    tau_K = sqrt(nu / (eps + 1e-30))
    tau_ratio = tau_t / (tau_K + 1e-30)
    eps_dyn = eps * (1 + dynCoeff * tanh(tau_ratio / dynRatioRef))
    // Vortex-stretching correction
    omega_mag = sqrt(eps / (C_mu * k + 1e-30))
    eps_vs = eps_dyn * (1 + vsCoeff * S_mag / (omega_mag + 1e-30))
    eps = clamp(eps_vs, epsilonMin, epsilonMax)

In OpenFOAM syntax::

    type        turbulentDissipationInlet10;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    C1          1.44;
...
__all__ = ["TurbulentDissipationInlet10BC"]


@BoundaryCondition.register("turbulentDissipationInlet10")
class TurbulentDissipationInlet10BC(BoundaryCondition):
    """v10 enhanced turbulent dissipation inlet with dynamic limiter.
    ...
    def apply(
        ...
        """Set boundary-face epsilon with dynamic limiter and vortex-stretching.
        ...
        if k is not None and nu is not None and nu > 0:
            ...
            if strain_rate is not None:
                ...
                epsilon = torch.max(epsilon, eps_prod * self._production_ratio)
        elif k is not None:
    ...
    def matrix_contributions(
        ...
        """Penalty method for v10 enhanced epsilon inlet BC."""
        device = get_device()
