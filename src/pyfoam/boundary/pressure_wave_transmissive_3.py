"""
Enhanced wave transmissive pressure boundary condition (v3).

Extends ``pressureWaveTransmissive2`` with spatially-varying relaxation,
Mach-number-dependent blending, and a turbulent-fluctuation damping term::

    Ma = |U| / c
    sigma = sigma_base * (1 + gamma * Ma^2)
    p_wave = p_owner - rho * c * (U_n - c) * dp / denom
    p_damp = damping * rho * k_turb
    p_face = (1 - blending * Ma) * p_wave + blending * Ma * p_far - p_damp

In OpenFOAM syntax::

    type        pressureWaveTransmissive3;
    phi         phi;
    rho         rho;
    psi         psi;
    gamma       1.4;
...
__all__ = ["PressureWaveTransmissive3BC"]


@BoundaryCondition.register("pressureWaveTransmissive3")
class PressureWaveTransmissive3BC(BoundaryCondition):
    """Enhanced pressure wave transmissive BC (v3) with Mach-dependent blending.
    ...
    def apply(
        ...
        """Apply v3 enhanced wave transmissive pressure BC.
        ...
        # Mach-dependent blending (more blending at higher Mach)
        blend_eff = self._blending * (1.0 + sigma * ma)
        p_face = (1.0 - blend_eff) * p_wave + blend_eff * self._field_inf

        # Turbulent fluctuation damping
        if k is not None:
            p_damp = self._damping * rho_val * k
    ...
    def matrix_contributions(
        ...
        """Relaxation-based matrix contribution for v3 wave transmissive."""
        device = get_device()
