"""
Enhanced outlet phase mean velocity boundary condition (v4).

Extends ``outletPhaseMeanVelocity3`` with a turbulent kinetic energy
coupling and a pressure-relaxation correction::

    alpha_safe = max(alpha, alphaMin)
    // Turbulent-flux weighting (from v3)
    ...
    // Turbulent kinetic energy coupling
    k_face = k_field (if provided) or k_local
    nut_face = C_mu * k_face^2 / (epsilon_face + 1e-30)
    U_tke = U_phase * (1 - tkeCoeff * nut_face / (nu + nut_face + 1e-30))
    // Pressure-relaxation correction
    p_relax = pressureRelax * (p_face - p_ref) / (rho * |Umean|^2 + 1e-30)
    U_outlet = U_tke * (1 - p_relax)
    // Mass conservation
    m_dot_target = rho * alpha_safe * Umean . n * A
    U_outlet *= m_dot_target / (m_dot_actual + 1e-30)
    U_outlet = clamp(|U_outlet|, 0, Umax) * direction

In OpenFOAM syntax::

    type              outletPhaseMeanVelocity4;
    Umean             uniform (1 0 0);
...
__all__ = ["OutletPhaseMeanVelocity4BC"]


@BoundaryCondition.register("outletPhaseMeanVelocity4")
class OutletPhaseMeanVelocity4BC(BoundaryCondition):
    """Enhanced outlet phase mean velocity BC v4 with TKE coupling.
    ...
    def apply(
        ...
        """Apply v4 enhanced outlet phase-mean velocity BC.
        ...
        # Pressure-gradient correction (from v2/v3)
    ...
    def matrix_contributions(
        ...
        """Penalty method for v4 outlet phase mean velocity BC."""
        device = get_device()
