"""
Enhanced turbulent dissipation inlet boundary condition (v7).

Extends ``turbulentDissipationInlet6`` with an adaptive production limiter
that uses the resolved turbulence intensity and a P/delta limiter::
    ...
    // Adaptive production limiter with P/delta correction
    S_ij = grad(U)  (mean strain rate)
    P_k = 2 * nut * S_ij : S_ij
    eps_prod = C1 * P_k * k / (k + 1e-30)
    eps_limited = max(eps_base, eps_prod * productionRatio)
    eps = clamp(eps_limited, epsilonMin, epsilonMax)

In OpenFOAM syntax::

    type        turbulentDissipationInlet7;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    C1          1.44;
...
__all__ = ["TurbulentDissipationInlet7BC"]


@BoundaryCondition.register("turbulentDissipationInlet7")
class TurbulentDissipationInlet7BC(BoundaryCondition):
    """v7 enhanced turbulent dissipation inlet with strain-rate-based production limiter.
    ...
    Coefficients:
        ...
        - ``alpha``: Base blending weight (default 1.0).
    ...
    def apply(
        ...
        """Set boundary-face epsilon with strain-rate production limiter.
        ...
        if k is not None and nu is not None and nu > 0:
            ...
            # Strain-rate-based production limiter
            if strain_rate is not None:
                nut_est = self._C_mu * k ** 2 / (eps_base + 1e-30)
                P_k = 2.0 * nut_est * strain_rate ** 2  # approximate production
                eps_prod = self._C1 * P_k
                epsilon = torch.max(eps_base, eps_prod * self._production_ratio)
            else:
                # Fallback: nut-based limiter
                nut_est = self._C_mu * k ** 2 / (eps_base + 1e-30)
    ...
    def matrix_contributions(
        ...
        """Penalty method for v7 enhanced epsilon inlet BC."""
        device = get_device()
