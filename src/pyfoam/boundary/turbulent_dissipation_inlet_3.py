"""
Enhanced turbulent dissipation inlet boundary condition (v3).

v3 extends v2 with a blending between intensity-based and
direct k-input epsilon estimates, plus wall-distance-aware scaling::

    k = 1.5 * (I * |U|)^2
    epsilon_intensity = C_mu^0.75 * k^1.5 / l_mix
    epsilon_k = C_mu^0.75 * k^1.5 / l_mix
    epsilon = alpha * epsilon_intensity + (1 - alpha) * epsilon_k

In OpenFOAM syntax::

    type        turbulentDissipationInlet3;
    mixingLength 0.01;
    Cmu         0.09;
    intensity   0.05;
    alpha       1.0;
...
__all__ = ["TurbulentDissipationInlet3BC"]


@BoundaryCondition.register("turbulentDissipationInlet3")
class TurbulentDissipationInlet3BC(BoundaryCondition):
    """v3 enhanced turbulent dissipation inlet with blending.

    Blends intensity-based and explicit-k epsilon estimates.
    ...
    Coefficients:
        ...
        - ``alpha``: Blending weight for intensity-based epsilon (default 1.0).
    ...
    def alpha(self) -> float:
        """Blending weight for intensity-based epsilon."""
        return self._alpha
    ...
    def apply(
        ...
        """Set boundary-face epsilon with blending.
        ...
        if k is not None:
            # Direct k-based epsilon
            eps_k = (self._C_mu ** 0.75) * (k ** 1.5) / (l + 1e-30)

            # If velocity also provided, blend with intensity-based estimate
            if velocity is not None and self._alpha < 1.0:
                ...
                epsilon = self._alpha * eps_intensity + (1.0 - self._alpha) * eps_k
            else:
                epsilon = eps_k
    ...
    def matrix_contributions(
        ...
        """Penalty method for v3 enhanced epsilon inlet BC."""
        device = get_device()
