"""
Enhanced turbulent frequency inlet boundary condition (v10).

Extends ``turbulentFrequencyInlet9`` with a dynamic time-scale correction
and a cross-diffusion term for improved k-omega SST compatibility::

    k = 1.5 * (I * |U|)^2
    // Two-layer model (from v9)
    omega_log = k^0.5 / (C_mu^0.25 * kappa * y)
    omega_buf = 6 * nu / (beta1 * y^2)
    omega_base = blend(y_plus) * omega_log + (1 - blend) * omega_buf
    // Kolmogorov-scale limiter (from v9)
    omega_kolm = sqrt(eps_kolm / (C_mu * k))
    omega = max(omega_base, omega_kolm)
    // Dynamic time-scale correction
    tau_t = 1 / (betaStar * omega + 1e-30)
    tau_K = sqrt(nu / (C_mu * k * omega + 1e-30))
    omega_dyn = omega * (1 + dynCoeff * tanh(tau_t / (tau_K + 1e-30) / dynRatioRef))
    // Cross-diffusion term (SST CD_kw)
    sigma_d = max(1 / (omega_dyn + 1e-30) * dOmega_dx, 0)
    omega_cd = omega_dyn + sigma_d / (betaStar * omega_dyn + 1e-30)
    omega = clamp(omega_cd, omegaMin, omegaMax)

In OpenFOAM syntax::

    type        turbulentFrequencyInlet10;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    beta1       0.075;
...
__all__ = ["TurbulentFrequencyInlet10BC"]


@BoundaryCondition.register("turbulentFrequencyInlet10")
class TurbulentFrequencyInlet10BC(BoundaryCondition):
    """v10 enhanced turbulent frequency inlet with dynamic correction and SST CD term.
    ...
    def apply(
        ...
        """Set boundary-face omega with dynamic correction and SST CD term.
        ...
        if k is not None and nu is not None and nu > 0:
            ...
            # Cross-diffusion term (SST CD_kw) — approximate gradient from face variation
            if n > 1:
    ...
    def matrix_contributions(
        ...
        """Penalty method for v10 enhanced omega inlet BC."""
        device = get_device()
