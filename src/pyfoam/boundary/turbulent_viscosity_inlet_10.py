"""
Enhanced turbulent viscosity inlet boundary condition (v10).

Extends ``turbulentViscosityInlet9`` with a Reynolds-stress transport
correction and a dynamic wall-function transition model::

    k = 1.5 * (I * |U|)^2
    epsilon = C_mu^0.75 * k^1.5 / l_mix
    nut_base = C_mu * k^2 / epsilon
    // Anisotropy (from v9)
    // Temperature correction (from v9)
    // Buffer-layer Van Driest (from v9)
    // Reynolds-stress transport correction
    P_k = 2 * nut * S^2
    tau_ij = rho * (P_k - epsilon) / (k + 1e-30)
    nut_rst = nut_base * (1 + rstCoeff * |tau_ij / (rho * k)|)
    // Dynamic wall-function transition
    y_plus = u_tau * y / nu
    blend_dyn = tanh((y_plus - yPlusTrans) / transWidth)
    nut = blend_dyn * nut_rst + (1 - blend_dyn) * nut_buf
    nut = clamp(nut, nutMin, nutMax)

In OpenFOAM syntax::

    type        turbulentViscosityInlet10;
    Cmu         0.09;
    intensity   0.05;
    lengthScale 0.01;
    kappa       0.41;
...
__all__ = ["TurbulentViscosityInlet10BC"]


@BoundaryCondition.register("turbulentViscosityInlet10")
class TurbulentViscosityInlet10BC(BoundaryCondition):
    """v10 enhanced turbulent viscosity inlet with RST correction and dynamic wall transition.
    ...
    Coefficients:
        ...
        - ``viscTempExp``: Temperature exponent for viscosity correction (default 0.5).
        - ``Tref``: Reference temperature for viscosity correction (K, default 300.0).
        - ``Abuf``: Van Driest buffer-layer damping constant (default 26.0).
        - ``rstCoeff``: Reynolds-stress transport correction coefficient (default 0.1).
    ...
    def __init__(self, patch: Patch, coeffs: dict[str, Any] | None = None) -> None:
        ...
        self._visc_temp_exp = float(self._coeffs.get("viscTempExp", 0.5))
        self._T_ref = float(self._coeffs.get("Tref", 300.0))
        self._A_buf = float(self._coeffs.get("Abuf", 26.0))
        self._rst_coeff = float(self._coeffs.get("rstCoeff", 0.1))
    ...
    @property
    def visc_temp_exp(self) -> float:
        """Temperature exponent for viscosity correction."""
        return self._visc_temp_exp

    @property
    def T_ref(self) -> float:
        """Reference temperature for viscosity correction (K)."""
        return self._T_ref

    @property
    def A_buf(self) -> float:
        """Van Driest buffer-layer damping constant."""
        return self._A_buf

    @property
    def rst_coeff(self) -> float:
        """Reynolds-stress transport correction coefficient."""
        return self._rst_coeff
    ...
    def apply(
        ...
        """Set boundary-face nut with RST correction and dynamic wall transition.
        ...
        Args:
            ...
            temperature: Current temperature (K) for viscosity correction.
        """
        device = field.device
        dtype = field.dtype
        ...
        if k is not None and epsilon is not None:
            ...
            if strain_rate is not None:
                tau_ij = (P_k - epsilon) / (k + 1e-30)
                nut_rst = nut_base * (1.0 + self._rst_coeff * torch.clamp(tau_ij.abs(), max=2.0))
            else:
                nut_rst = nut_base
        ...
        if temperature is not None and temperature > 0:
            T_ratio = (temperature / self._T_ref) ** self._visc_temp_exp
            nut_rst = nut_rst * T_ratio

        # Wall-distance blending with buffer-layer model and dynamic transition
        if nu is not None and nu > 0:
            k_for_wall = k if k is not None else nut_rst / (self._C_mu + 1e-30) * 0.1
            u_tau = (self._C_mu ** 0.25) * torch.sqrt(torch.clamp(k_for_wall, min=1e-30))
            y_plus = u_tau * self._wall_dist / nu

            # Buffer-layer Van Driest model
            nut_buf = self._kappa * u_tau * self._wall_dist * (
                1.0 - torch.exp(-y_plus / self._A_buf)
            ) ** 2
            ...
            nut = blend_dyn * nut_sc + (1.0 - blend_dyn) * nut_buf
        else:
            nut = nut_rst
    ...
    def matrix_contributions(
        ...
        """Penalty method for v10 nut inlet BC."""
        device = get_device()
