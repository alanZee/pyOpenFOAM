"""Enhanced processor cyclic boundary condition (v3).

In OpenFOAM syntax::

    type        processorCyclic3;
    value       uniform 0;

Coefficients:
    - Standard processor cyclic parameters (from base and earlier versions).
    - ``transform_correct_coeff`` (float): Transform correction factor. (default 0.05).
"""
...
__all__ = ["ProcessorCyclic3BC"]


@BoundaryCondition.register("processorCyclic3")
class ProcessorCyclic3BC(BoundaryCondition):
    """Enhanced processor cyclic v3.
    ...
    def apply(
        ...
        """Apply enhanced processor cyclic v3."""
        device = field.device
        dtype = field.dtype
        ...
        # v3 enhancement: transform correction factor.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v3 enhanced processor cyclic BC."""
        device = get_device()
