"""
Enhanced turbulent frequency inlet boundary condition (v12).

Extends ``turbulentFrequencyInlet11`` with a compressibility-aware
correction and enhanced SST limiter with pressure-gradient sensitivity::

    k = 1.5 * (I * |U|)^2
    // Two-layer model (from v11)
    // Kolmogorov-scale limiter (from v11)
    // Dynamic time-scale correction (from v11)
    // Cross-diffusion term (from v11)
    // Frequency-dependent blending (from v11)
    // Compressibility correction
    Ma_t = sqrt(2 * k) / c
    Ma_eff = clamp(Ma_t, 0, MaLimit)
    omega_comp = omega * (1 + compCoeff * Ma_eff^2)
    // Pressure-gradient sensitivity
    dp_dx_norm = |dp/dx| / (rho * |U|^2 + 1e-30)
    omega_pg = omega_comp * (1 + pgCoeff * dp_dx_norm)
    // Enhanced SST limiter (from v11)
    omega = max(omega_pg, omega_sst)
    omega = clamp(omega, omegaMin, omegaMax)

In OpenFOAM syntax::

    type        turbulentFrequencyInlet12;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    beta1       0.075;
...
__all__ = ["TurbulentFrequencyInlet12BC"]


@BoundaryCondition.register("turbulentFrequencyInlet12")
class TurbulentFrequencyInlet12BC(BoundaryCondition):
    """v12 enhanced turbulent frequency inlet with compressibility and pressure-gradient.
    ...
    def apply(
        ...
        """Set boundary-face omega with compressibility and pressure-gradient corrections.
        ...
        if k is not None and nu is not None and nu > 0:
            ...
            omega = torch.max(omega_comp, omega_sst)
        elif k is not None:
    ...
    def matrix_contributions(
        ...
        """Penalty method for v12 enhanced omega inlet BC."""
        device = get_device()
