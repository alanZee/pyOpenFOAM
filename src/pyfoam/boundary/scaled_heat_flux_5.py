"""
Enhanced scaled heat flux boundary condition (v5).

Extends ``scaledHeatFlux4`` with temperature-dependent emissivity
and an enhanced conjugate interface model::
    ...
    eps_T = epsilonSigma * (1 + betaEps * (T_mean - Tamb))
    q_rad = eps_T * sigma_sb * (T_mean^4 - T_amb^4)
    // Enhanced conjugate interface model (from v4)
    T_interface = (k * T_interior + h_conv * T_fluid) / (k + h_conv + 1e-30)
    q_cond = k * (T_interface - T_face) / delta_n
    // Contact resistance correction
    R_contact = contactResistance / A_face
    q_contact = (T_interior - T_face) / (R_contact + 1e-30)
    q_total = blendCoeff * q_conv + (1 - blendCoeff) * q_cond - q_rad + contactCoeff * q_contact
    dT/dn = -q_total / k_eff

In OpenFOAM syntax::

    type        scaledHeatFlux5;
    scale       2.0;
    q_ref       500.0;
    k           0.025;
    alphaT      0.0;
...
__all__ = ["ScaledHeatFlux5BC"]


@BoundaryCondition.register("scaledHeatFlux5")
class ScaledHeatFlux5BC(BoundaryCondition):
    """Enhanced scaled heat flux BC v5 with temperature-dependent emissivity.
    ...
    def apply(
        ...
        """Apply enhanced scaled heat flux BC v5 with temperature-dependent emissivity.
    ...
    def matrix_contributions(
        ...
        """Matrix contributions for enhanced scaled heat flux BC v5."""
        device = get_device()
