"""
Enhanced turbulent dissipation inlet boundary condition (v12).

Extends ``turbulentDissipationInlet11`` with a compressibility-aware
correction and an enhanced vortex-stretching model::

    k = 1.5 * (I * |U|)^2
    // Two-layer model (from v11)
    // Kolmogorov-scale limiter (from v11)
    // Dynamic time-scale limiter (from v11)
    // Anisotropic dissipation (from v11)
    // Spectral cascade model (from v11)
    // Compressibility correction
    Ma_t = sqrt(2 * k) / c
    Ma_eff = clamp(Ma_t, 0, MaLimit)
    eps_comp = eps * (1 + compCoeff * Ma_eff^2)
    // Enhanced vortex-stretching with Q-criterion
    Q = 0.5 * (Omega^2 - S^2)
    vs_enhanced = vsCoeff * max(Q, 0) / (eps / k + 1e-30)
    eps_out = eps_comp * (1 + vs_enhanced)
    eps_out = clamp(eps_out, epsilonMin, epsilonMax)

In OpenFOAM syntax::

    type        turbulentDissipationInlet12;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    C1          1.44;
...
__all__ = ["TurbulentDissipationInlet12BC"]


@BoundaryCondition.register("turbulentDissipationInlet12")
class TurbulentDissipationInlet12BC(BoundaryCondition):
    """v12 enhanced turbulent dissipation inlet with compressibility correction.
    ...
    def apply(
        ...
        """Set boundary-face epsilon with compressibility and enhanced vortex-stretching.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v12 enhanced epsilon inlet BC."""
        device = get_device()
