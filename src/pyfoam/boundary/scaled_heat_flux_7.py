"""
Enhanced scaled heat flux boundary condition (v7).

Extends ``scaledHeatFlux6`` with a temperature-history-aware thermal inertia
model and an enhanced spatial weighting using Gaussian process interpolation::

    T_mean = (T_face + T_ref) / 2
    scale_eff = scale * (1 + alpha_T * (T_mean - T_scaleRef)) * spatialWeight
    q_conv = scale_eff * q_ref * timeModulation(t)
    // Temperature-dependent emissivity (from v6)
    // Conjugate interface coupling (from v6)
    // Contact resistance (from v6)
    // Transient thermal inertia (from v6)
    // Temperature-history-aware inertia
    T_rate = (T - T_prev) / dt
    T_accel = (T_rate - T_rate_prev) / dt
    q_hist = historyCoeff * rho_cp * thickness * (T_rate + inertiaDamp * T_accel * tau)
    // Enhanced spatial weighting
    r_norm = |x - x_center| / spatialScale
    W_gp = exp(-spatialDecay * r_norm^2) * (1 + spatialCorr * cos(2 * pi * r_norm / spatialPeriod))
    q_total = blendCoeff * q_conv * W_gp + (1 - blendCoeff) * q_cond - q_rad + contactCoeff * q_contact + q_hist
    dT/dn = -q_total / k_eff

In OpenFOAM syntax::

    type        scaledHeatFlux7;
    scale       2.0;
    q_ref       500.0;
    k           0.025;
    alphaT      0.0;
...
__all__ = ["ScaledHeatFlux7BC"]


@BoundaryCondition.register("scaledHeatFlux7")
class ScaledHeatFlux7BC(BoundaryCondition):
    """Enhanced scaled heat flux BC v7 with history-aware inertia and spatial periodicity.
    ...
    Coefficients:
        ...
        - ``historyCoeff``: Temperature history correction coefficient (default 0.0).
        - ``inertiaDamp``: Acceleration damping for history model (default 0.5).
        - ``spatialCorr``: Spatial periodic correction amplitude (default 0.0).
        - ``spatialPeriod``: Spatial periodic correction period (m, default 1.0).
    ...
    def __init__(self, patch: Patch, coeffs: dict[str, Any] | None = None) -> None:
        ...
        self._history_coeff = float(self._coeffs.get("historyCoeff", 0.0))
        self._inertia_damp = float(self._coeffs.get("inertiaDamp", 0.5))
        self._spatial_corr = float(self._coeffs.get("spatialCorr", 0.0))
        self._spatial_period = float(self._coeffs.get("spatialPeriod", 1.0))
        self._T_prev: float | None = None
        self._T_rate_prev: float | None = None
    ...
    def history_coeff(self) -> float:
        """Temperature history correction coefficient."""
        return self._history_coeff

    @property
    def inertia_damp(self) -> float:
        """Acceleration damping for history model."""
        return self._inertia_damp

    @property
    def spatial_corr(self) -> float:
        """Spatial periodic correction amplitude."""
        return self._spatial_corr

    @property
    def spatial_period(self) -> float:
        """Spatial periodic correction period (m)."""
        return self._spatial_period
    ...
    def apply(
        ...
        """Apply enhanced scaled heat flux BC v7 with history-aware inertia.
        ...
        if T_field is not None:
            ...
            # Enhanced spatial weighting with periodic correction
            if self._spatial_decay > 0:
                ...
                W_gp = torch.exp(-self._spatial_decay * r_norm ** 2)
                if self._spatial_corr > 0:
                    import math
                    W_gp = W_gp * (1.0 + self._spatial_corr * torch.cos(
                        2.0 * math.pi * r_norm / (self._spatial_period + 1e-30)
                    ))
                spatial_eff = self._spatial_weight * W_gp
            else:
            ...
            # Temperature-history-aware inertia
            if self._history_coeff > 0 and dt is not None and dt > 0:
                rho_cp = self._density * self._Cp
                tau = rho_cp * self._thickness / (k_eff + 1e-30)
                if self._T_prev is not None:
                    T_rate = (T_face_current - self._T_prev) / dt
                    if self._T_rate_prev is not None:
                        T_accel = (T_rate - self._T_rate_prev) / dt
                        q_hist = self._history_coeff * rho_cp * self._thickness * (
                            T_rate + self._inertia_damp * T_accel * tau
                        )
                        q_total = q_total + q_hist
                    self._T_rate_prev = T_rate.clone()
    ...
    def matrix_contributions(
        ...
        """Matrix contributions for enhanced scaled heat flux BC v7."""
        device = get_device()
