"""
Enhanced turbulent frequency inlet boundary condition (v11).

Extends ``turbulentFrequencyInlet10`` with a frequency-dependent blending
and an enhanced cross-diffusion model for SST compatibility::

    k = 1.5 * (I * |U|)^2
    // Two-layer model (from v10)
    // Kolmogorov-scale limiter (from v10)
    // Dynamic time-scale correction (from v10)
    // Cross-diffusion term (from v10)
    // Frequency-dependent blending
    omega_ratio = omega / (omegaMin + 1e-30)
    freq_blend = clamp(log(1 + omega_ratio) / freqBlendScale, 0, 1)
    omega_blend = freq_blend * omega_dyn + (1 - freq_blend) * omega_base
    // Enhanced SST limiter with curvature correction
    F2 = 1 / (1 + (kappa * y / (C_mu^0.25 * sqrt(k) * yPlusBlend))^2)
    omega_sst = F2 * sqrt(k) / (C_mu^0.25 * kappa * y)
    omega = max(omega_blend, omega_sst)
    omega = clamp(omega, omegaMin, omegaMax)

In OpenFOAM syntax::

    type        turbulentFrequencyInlet11;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    beta1       0.075;
...
__all__ = ["TurbulentFrequencyInlet11BC"]


@BoundaryCondition.register("turbulentFrequencyInlet11")
class TurbulentFrequencyInlet11BC(BoundaryCondition):
    """v11 enhanced turbulent frequency inlet with frequency-dependent blending.
    ...
    def apply(
        ...
        """Set boundary-face omega with frequency-dependent blending and enhanced SST limiter.
        ...
        if k is not None and nu is not None and nu > 0:
            ...
            u_tau = (self._C_mu ** 0.25) * torch.sqrt(k)
            y_plus_loc = u_tau * self._wall_dist / (nu + 1e-30)
            ...
            omega = torch.max(omega_blend, omega_sst)
        elif k is not None:
    ...
    def matrix_contributions(
        ...
        """Penalty method for v11 enhanced omega inlet BC."""
        device = get_device()
