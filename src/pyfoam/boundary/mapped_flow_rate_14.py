"""Enhanced mapped flow rate boundary condition (v14).

In OpenFOAM syntax::

    type        mappedFlowRate14;
    value       uniform 0;

Coefficients:
    - Standard mapped flow rate parameters (from base and earlier versions).
    - ``radial_pressure_grad`` (float): Radial pressure gradient correction. (default 0.02).
"""
...
__all__ = ["MappedFlowRate14BC"]


@BoundaryCondition.register("mappedFlowRate14")
class MappedFlowRate14BC(BoundaryCondition):
    """Enhanced mapped flow rate v14.
    ...
    def apply(
        ...
        """Apply enhanced mapped flow rate v14."""
        device = field.device
        dtype = field.dtype
        ...
        # v14 enhancement: radial pressure gradient correction.
    ...
    def matrix_contributions(
        ...
        """Penalty method for v14 enhanced mapped flow rate BC."""
        device = get_device()
