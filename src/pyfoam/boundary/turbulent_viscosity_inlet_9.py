"""
Enhanced turbulent viscosity inlet boundary condition (v9).

Extends ``turbulentViscosityInlet8`` with a temperature-dependent viscosity
correction and enhanced wall-function blending::

    k = 1.5 * (I * |U|)^2
    epsilon = C_mu^0.75 * k^1.5 / l_mix
    nut_base = C_mu * k^2 / epsilon
    // Anisotropy correction (from v8)
    // Wall-distance blending (from v8)
    // Temperature-dependent viscosity correction
    T_ratio = (T / Tref)^viscTempExp
    nut_T = nut_base * T_ratio
    // Enhanced wall-function blending with buffer-layer model
    y_plus = u_tau * y / nu
    nut_buf = kappa * u_tau * y * (1 - exp(-y_plus / A_buf))^2
    nut = blend * nut_T + (1 - blend) * nut_buf
    nut = clamp(nut, nutMin, nutMax)

In OpenFOAM syntax::

    type        turbulentViscosityInlet9;
    Cmu         0.09;
    intensity   0.05;
    lengthScale 0.01;
    kappa       0.41;
...
__all__ = ["TurbulentViscosityInlet9BC"]


@BoundaryCondition.register("turbulentViscosityInlet9")
class TurbulentViscosityInlet9BC(BoundaryCondition):
    """v9 enhanced turbulent viscosity inlet with temperature correction.
    ...
    def apply(
        ...
        """Set boundary-face nut with temperature correction and buffer-layer blending.
        ...
        if temperature is not None and temperature > 0:
            ...
            nut_base = nut_base * T_ratio

        # Wall-distance blending with buffer-layer model
        if nu is not None and nu > 0:
            ...
            nut = blend * nut_sc + (1.0 - blend) * nut_buf
        else:
            nut = nut_base
    ...
    def matrix_contributions(
        ...
        """Penalty method for v9 nut inlet BC."""
        device = get_device()
