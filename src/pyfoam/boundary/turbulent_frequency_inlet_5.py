"""
Enhanced turbulent frequency inlet boundary condition (v5).

Extends v4 with a two-layer model that estimates omega differently in the
log-law and buffer layers, plus a turbulent production limiter::
    ...
    omega_log = k^0.5 / (C_mu^0.25 * kappa * y + 1e-30)
    omega_buf = 6 * nu / (beta1 * y^2 + 1e-30)
    omega = blend(y_plus, yPlusLow, yPlusHigh) * omega_log
            + (1 - blend) * omega_buf
    omega = clamp(omega, omegaMin, omegaMax)

In OpenFOAM syntax::

    type        turbulentFrequencyInlet5;
    mixingLength 0.01;
    Cmu         0.09;
    kappa       0.41;
    beta1       0.075;
...
__all__ = ["TurbulentFrequencyInlet5BC"]


@BoundaryCondition.register("turbulentFrequencyInlet5")
class TurbulentFrequencyInlet5BC(BoundaryCondition):
    """v5 enhanced turbulent frequency inlet with two-layer blending.
    ...
    def apply(
        ...
        """Set boundary-face omega with two-layer model and clamping.
        ...
        if k is not None and nu is not None and nu > 0:
            omega = self._two_layer_omega(k, self._wall_dist, nu, n, device, dtype)
        elif k is not None:
    ...
    def matrix_contributions(
        ...
        """Penalty method for v5 enhanced omega inlet BC."""
        device = get_device()
