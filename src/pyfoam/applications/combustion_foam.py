"""
combustionFoam — general combustion solver with multiple reaction mechanisms.

Implements a general-purpose combustion solver supporting multiple reaction
mechanisms (Arrhenius, eddy-dissipation, and combined EDC):

    ∂(ρYi)/∂t + ∇·(ρUYi) = ∇·(ρDi∇Yi) + ωi

where ωi depends on the selected mechanism:

- **Arrhenius**: ωi = νi · A · T^β · exp(-Ea/RT) · ∏[Yj^νj]
- **EddyDissipation**: ωi = ρ · A · ε/k · min(Yf, Yox/S)
- **EDC** (Eddy Dissipation Concept): Arrhenius * eddy-dissipation factor

Reads:
- ``0/YA``, ``0/YB``, ... — mass fractions for each species
- ``0/T`` — temperature field
- ``0/U`` — velocity field
- ``0/p`` — pressure field
- ``constant/polyMesh`` — mesh
- ``constant/thermophysicalProperties`` — thermodynamic properties
- ``constant/combustionProperties`` — mechanism type and parameters
...
Usage::
    ...
    solver = CombustionFoam("path/to/case")
    solver.run()
"""
...
__all__ = ["CombustionFoam", "CombustionReaction", "MechanismType"]

logger = logging.getLogger(__name__)


class MechanismType(Enum):
    """Supported combustion mechanism types.

    - **ARRHENIUS**: Pure Arrhenius kinetics.
    - **EDDY_DISSIPATION**: Magnussen-Hjertager eddy-dissipation model.
    - **EDC**: Eddy Dissipation Concept (combined Arrhenius + mixing).
    """
    ARRHENIUS = "Arrhenius"
    EDDY_DISSIPATION = "EddyDissipation"
    EDC = "EDC"


@dataclass
class CombustionReaction:
    """A single combustion reaction.
    ...
    beta : float
        Temperature exponent (Arrhenius).
    Ea : float
        Activation energy (J/mol, Arrhenius).
    reactants : dict[str, float]
        Stoichiometric coefficients for reactants.
    products : dict[str, float]
        Stoichiometric coefficients for products.
    fuel : str
        Name of the fuel species (for eddy-dissipation).
    oxidiser : str
        Name of the oxidiser species.
    stoich_ratio : float
        Stoichiometric oxidiser-to-fuel mass ratio.
    """
    name: str = ""
    A: float = 1.0
    beta: float = 0.0
...
class CombustionFoam(SolverBase):
    """General combustion solver with multiple reaction mechanisms.

    Supports Arrhenius, eddy-dissipation (Magnussen-Hjertager), and
    combined EDC (Eddy Dissipation Concept) combustion models.
    ...
    reactions : list[CombustionReaction]
        List of reactions.
    """
    ...
    def __init__(self, case_path: Union[str, Path]) -> None:
        ...
        # Read combustion properties (mechanism type, model constants)
        self._read_combustion_properties()
        ...
        logger.info(
            "CombustionFoam ready: %d species, %d reactions, mechanism=%s",
            len(self.species), len(self.reactions), self.mechanism.value,
        )
    ...
    def _read_combustion_properties(self) -> None:
        """Read combustion properties from constant/combustionProperties."""
        # Defaults
        self.mechanism = MechanismType.ARRHENIUS
        self.eddy_dissipation_A = 4.0   # Magnussen constant
        self.eddy_dissipation_B = 0.5   # Magnussen constant

        cp_path = self.case_path / "constant" / "combustionProperties"
        if cp_path.exists():
            try:
                from pyfoam.io.dictionary import parse_dict_file
                cp = parse_dict_file(cp_path)

                mech_str = str(cp.get("mechanism", "Arrhenius"))
                try:
                    self.mechanism = MechanismType(mech_str)
                except ValueError:
                    logger.warning(
                        "Unknown mechanism '%s', defaulting to Arrhenius", mech_str
                    )
                    self.mechanism = MechanismType.ARRHENIUS

                self.eddy_dissipation_A = float(
                    cp.get("eddyDissipationA", 4.0)
                )
                self.eddy_dissipation_B = float(
                    cp.get("eddyDissipationB", 0.5)
                )
            except Exception as e:
                logger.warning("Could not read combustion properties: %s", e)
    ...
    def _read_reactions(self) -> list[CombustionReaction]:
        """Read reaction mechanism from constant/reactions."""
        reactions: list[CombustionReaction] = []

        rxn_path = self.case_path / "constant" / "reactions"
        if not rxn_path.exists():
            logger.warning("No reactions file found, using default fuel+ox→prod")
            reactions.append(CombustionReaction(
                name="reaction1",
                A=1.0, beta=0.0, Ea=0.0,
                reactants={"fuel": 1.0, "O2": 1.0},
                products={"prod": 1.0},
                fuel="fuel", oxidiser="O2", stoich_ratio=1.0,
            ))
            return reactions
        ...
        try:
            ...
            for key, value in rxn_dict.items():
                if isinstance(value, dict):
                    rxn = CombustionReaction(name=key)
                    ...
                    rxn.fuel = str(value.get("fuel", ""))
                    rxn.oxidiser = str(value.get("oxidiser", ""))
                    rxn.stoich_ratio = float(value.get("stoichRatio", 1.0))
    ...
    def _read_fv_solution_settings(self) -> None:
        ...
        self.convergence_tolerance = float(
            fv.get_path("combustionFoam/convergenceTolerance", 1e-4)
        )
    ...
    def _init_fields(
        ...
        if not self.species:
            self.species = ["fuel", "O2", "prod"]
            Y["fuel"] = torch.full((n_cells,), 0.05, dtype=dtype, device=device)
            Y["O2"] = torch.full((n_cells,), 0.23, dtype=dtype, device=device)
            Y["prod"] = torch.zeros(n_cells, dtype=dtype, device=device)
    ...
    def _compute_eddy_dissipation_rate(
        self,
        reaction: CombustionReaction,
        T: torch.Tensor,
        Y: dict[str, torch.Tensor],
        ...
        """Compute eddy-dissipation (Magnussen-Hjertager) reaction rate.

        Rate = rho * A * (epsilon/k) * min(Y_fuel, Y_ox / S)

        Falls back to a simplified mixing-time estimate when k/epsilon
        ...
        fuel = reaction.fuel
        oxidiser = reaction.oxidiser
        S = reaction.stoich_ratio

        if fuel not in Y or oxidiser not in Y:
            return torch.zeros(n_cells, dtype=dtype, device=device)

        Y_fuel = Y[fuel].clamp(min=0.0)
        Y_ox = Y[oxidiser].clamp(min=0.0)

        # Mixing time: prefer k-epsilon, otherwise use a fixed estimate
        if k_field is not None and epsilon_field is not None:
            k_safe = k_field.clamp(min=1e-10)
            eps_safe = epsilon_field.clamp(min=1e-10)
            tau_mix = k_safe / eps_safe
        else:
            # Simplified estimate: tau_mix ~ 1e-3 s
            tau_mix = torch.full(
                (n_cells,), 1e-3, dtype=dtype, device=device,
            )

        rate = self.eddy_dissipation_A * Y_fuel * Y_ox / (
            tau_mix * (1.0 + S)
        )

        return rate

    def _compute_edc_rate(
        self,
        reaction: CombustionReaction,
        T: torch.Tensor,
        Y: dict[str, torch.Tensor],
        ...
        """Compute EDC (Eddy Dissipation Concept) reaction rate.

        EDC combines Arrhenius kinetics with a mixing-limited rate:

            Rate = Arrhenius_rate * min(1, mixing_factor)

        The mixing factor scales with the local turbulent mixing time.
        """
        arrhenius_rate = self._compute_arrhenius_rate(reaction, T, Y)
        ed_rate = self._compute_eddy_dissipation_rate(
            reaction, T, Y, k_field, epsilon_field,
        )

        # EDC: rate limited by the slower of the two
        return torch.min(arrhenius_rate, ed_rate)

    def _compute_combustion_source_terms(
        self,
        T: torch.Tensor,
        Y: dict[str, torch.Tensor],
        k_field: torch.Tensor | None = None,
        ...
        """Compute combustion source terms for all species.

        Dispatches to the selected mechanism.
        """
        device = T.device
        dtype = T.dtype
        n_cells = T.shape[0]
        ...
        for reaction in self.reactions:
            # Select mechanism
            if self.mechanism == MechanismType.ARRHENIUS:
                rate = self._compute_arrhenius_rate(reaction, T, Y)
            elif self.mechanism == MechanismType.EDDY_DISSIPATION:
                rate = self._compute_eddy_dissipation_rate(
                    reaction, T, Y, k_field, epsilon_field,
                )
            elif self.mechanism == MechanismType.EDC:
                rate = self._compute_edc_rate(
                    reaction, T, Y, k_field, epsilon_field,
                )
            else:
                rate = self._compute_arrhenius_rate(reaction, T, Y)
    ...
    def _compute_heat_release(
        ...
        Simplified: assumes constant heat of reaction.
        """
        n_cells = T.shape[0]
        device = T.device
        dtype = T.dtype
    ...
    def _assemble_species_equation(
        ...
        d(rho Yi)/dt + div(rho U Yi) = div(rho Di grad Yi) + omega_i
        """
        device = get_device()
        dtype = get_default_dtype()
        n_cells = self.mesh.n_cells
        ...
        if n_faces > n_internal:
            ...
            diag_diff = diag_diff + scatter_add(
                bnd_coeff / bnd_V, owner[n_internal:], n_cells,
            )
    ...
    def _assemble_temperature_equation(
        ...
        rho*Cp*dT/dt + rho*Cp*div(UT) = div(lambda*grad(T)) + Q
        """
        device = get_device()
        dtype = get_default_dtype()
        n_cells = self.mesh.n_cells
    ...
    def run(self) -> dict[str, Any]:
        ...
        logger.info("Starting combustionFoam run (mechanism=%s)", self.mechanism.value)
        ...
        for t, step in time_loop:
            ...
            # Compute combustion source terms
            omega = self._compute_combustion_source_terms(self.T, self.Y)
        ...
        logger.info("combustionFoam completed")
        logger.info(
            "  T range: [%.1f, %.1f] K",
