"""
pisoFoam — transient incompressible solver with turbulence support.

Implements the PISO algorithm for transient incompressible Navier-Stokes
equations with optional RANS turbulence modelling.

This is the transient counterpart of :class:`SimpleFoam`:

    read case → build mesh → initialise fields → PISO time loop → write results
...
class PisoFoam(SolverBase):
    """Transient incompressible PISO solver with turbulence support.
    ...
    def __init__(self, case_path: Union[str, Path]) -> None:
        ...
        logger.info("PisoFoam ready: nu=%.6e, Re~%.0f", self.nu, 1.0 / max(self.nu, 1e-30))
            ...
            try:
                ...
                if simulation_type == "RAS":
                    ...
                    if isinstance(ras_dict, dict):
                        ...
                        # RAS block might be nested differently
                        model_name = "kEpsilon"
            except Exception:
                pass
    ...
    def _build_solver(self) -> PISOSolver:
        ...
        config = PISOConfig(
            ...
            # PISO uses no under-relaxation (transient)
    ...
    def run(self) -> ConvergenceData:
        ...
        logger.info("Starting pisoFoam run")
        ...
        if last_convergence is not None:
            if last_convergence.converged:
                logger.info("pisoFoam completed successfully (converged)")
            else:
                logger.warning("pisoFoam completed without full convergence")
