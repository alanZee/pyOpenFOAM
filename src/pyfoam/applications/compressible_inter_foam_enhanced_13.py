"""
compressibleInterFoamEnhanced13 -- enhanced compressible two-phase VOF solver v13.

Extends :class:`CompressibleInterFoamEnhanced12` with coupling algorithm variants.

Usage::

    from pyfoam.applications.compressible_inter_foam_enhanced_13 import CompressibleInterFoamEnhanced13

    solver = CompressibleInterFoamEnhanced13("path/to/case", simplec=True, coupled=True)
    solver.run()
"""
...
from .compressible_inter_foam_enhanced_12 import CompressibleInterFoamEnhanced12
from .time_loop import TimeLoop
from .convergence import ConvergenceMonitor

__all__ = ["CompressibleInterFoamEnhanced13"]

logger = logging.getLogger(__name__)


class CompressibleInterFoamEnhanced13(CompressibleInterFoamEnhanced12):
    """Enhanced compressible two-phase VOF solver v13.
    ...
    def __init__(self, case_path: Union[str, Path], simplec: bool = True,
        ...
        logger.info("CompressibleInterFoamEnhanced13 ready: simplec=%s, coupled=%s", self.simplec, self.coupled)
    ...
    def _coupled_solve(self, U, p, rho, dt):
        if not self.coupled:
            return U, p
        mesh = self.mesh
        n_cells = mesh.n_cells
    ...
    def run(self) -> ConvergenceData:
        ...
        logger.info("Starting compressibleInterFoamEnhanced13 run")
        ...
        for t, step in time_loop:
            ...
            if self.coupled:
                self.U, self.p = self._coupled_solve(self.U, self.p, rho, self.delta_t)
