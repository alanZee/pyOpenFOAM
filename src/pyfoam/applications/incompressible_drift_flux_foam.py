"""
incompressibleDriftFlux — Incompressible drift-flux solver.

Implements the OpenFOAM-style ``incompressibleDriftFluxFoam`` solver for
two-phase flows with algebraic slip modelling. The mixture momentum
equation is solved for the bulk velocity, while the dispersed phase
fraction is transported with a drift-flux correction.

Algebraic slip model:
- Slip (drift) velocity follows Stokes settling:
  ``U_slip = (rho_p - rho_c) * g * d^2 / (18 * mu_c)``
  where p = dispersed (particle) phase, c = carrier (continuous) phase.
- Mixture density: ``rho_m = alpha * rho_d + (1 - alpha) * rho_c``
- Mixture viscosity: ``mu_m = alpha * mu_d + (1 - alpha) * mu_c``

Governing equations:

- Continuity (mixture): ``∇·U_m = 0``
- Momentum (mixture):
  ``∂(ρ_m U_m)/∂t + ∇·(ρ_m U_m U_m) = -∇p + ∇·(μ_m ∇U_m) + ρ_m g``
- Phase transport:
  ``∂α/∂t + ∇·(α U_m) + ∇·(α(1-α) U_slip) = 0``

The slip velocity couples the phases through the drift-flux relative
velocity term ``α(1-α) U_slip``, representing momentum exchange without
solving separate momentum equations for each phase.
...
__all__ = ["IncompressibleDriftFluxFoam"]

logger = logging.getLogger(__name__)


class IncompressibleDriftFluxFoam(SolverBase):
    """Incompressible drift-flux solver with algebraic slip model.
    ...
    d : float
        Particle diameter (m) for Stokes settling velocity.
    g : tuple[float, float, float]
        Gravity vector (m/s^2).
    C_alpha : float
        Compression coefficient for phase-fraction transport (controls
        anti-diffusive sharpening at the interface).
    """
    ...
    def __init__(
        ...
        self.mu_d = mu_d
        self.d = d
        self.g = torch.tensor(g, dtype=get_default_dtype(), device=get_device())
        ...
        # 计算 Stokes 沉降速度标量
        # U_slip = (rho_d - rho_c) * g_mag * d^2 / (18 * mu_c)
        g_mag = float(torch.norm(self.g).item())
        self.U_slip_scalar = (
            (self.rho_d - self.rho_c) * g_mag * self.d ** 2
            / (18.0 * self.mu_c)
        )

        logger.info(
            "IncompressibleDriftFluxFoam ready: rho_c=%.1f, rho_d=%.1f, "
            "d=%.2e, U_slip_scalar=%.6e",
            rho_c, rho_d, d, self.U_slip_scalar,
        )
    ...
    def _compute_mixture_rho(self, alpha: torch.Tensor) -> torch.Tensor:
        """混合密度：rho_m = alpha * rho_d + (1 - alpha) * rho_c。"""
        return alpha * self.rho_d + (1.0 - alpha) * self.rho_c

    def _compute_mixture_mu(self, alpha: torch.Tensor) -> torch.Tensor:
        """混合粘度：mu_m = alpha * mu_d + (1 - alpha) * mu_c。"""
        return alpha * self.mu_d + (1.0 - alpha) * self.mu_c

    # ------------------------------------------------------------------
    # 漂移速度
    # ------------------------------------------------------------------

    def _compute_drift_velocity(self, alpha: torch.Tensor) -> torch.Tensor:
        """计算漂移（滑移）速度矢量。

        U_drift = alpha * (1 - alpha) * U_slip_vector
        方向沿重力方向（沉降方向）。
        """
        # 沉降方向：沿重力方向
        # 对于沉降颗粒，U_slip 方向与 g 相同（向下）
        U_slip_vec = self.U_slip_scalar * self.g / torch.norm(self.g).clamp(min=1e-30)
        # drift flux = alpha * (1 - alpha) * U_slip
        drift = alpha.unsqueeze(-1) * (1.0 - alpha).unsqueeze(-1) * U_slip_vec.unsqueeze(0)
        return drift
    ...
    def run(self) -> ConvergenceData:
        ...
        logger.info("Starting incompressibleDriftFlux run")
    ...
    def _pimple_drift_flux_step(self) -> tuple[torch.Tensor, ...]:
        """单时间步：相分数输运 + PIMPLE 压力-速度耦合。
        ...
        # 阶段 1：相分数输运（含漂移通量）
        # ============================================================
        alpha = self._advect_alpha(alpha, U, phi)
        ...
        for outer in range(n_outer):
            ...
            if outer % 5 == 0 or outer < 3:
                logger.info(
                    "driftFlux outer %d: U_res=%.6e, p_res=%.6e, "
                    "cont=%.6e",
    ...
    def _advect_alpha(
        ...
        """输运相分数：∂α/∂t + ∇·(α U_m) + ∇·(α(1-α) U_slip) = 0。
        ...
        # 对流通量（迎风）
        alpha_upwind = torch.where(flux >= 0, alpha_P, alpha_N)
        conv_flux = flux * alpha_upwind

        # 漂移通量
        drift_vel = self._compute_drift_velocity(alpha)
        w = mesh.face_weights[:n_internal]
        drift_face = (
            w.unsqueeze(-1) * drift_vel[int_owner]
            + (1.0 - w).unsqueeze(-1) * drift_vel[int_neigh]
        )
        face_areas = mesh.face_areas[:n_internal]
        # 漂移通量在面上的法向分量
        drift_flux = (drift_face * face_areas).sum(dim=1)

        # 压缩项：C_alpha * ∇·(alpha(1-alpha) n_f |phi|)
        # 面法向
        face_norm = face_areas / face_areas.norm(dim=1, keepdim=True).clamp(min=1e-30)
        alpha_diff = alpha_P - alpha_N
        # 压缩通量：限制在界面附近
        alpha_bar = 0.5 * (alpha_P + alpha_N)
        compressive = (
            self.C_alpha * flux.abs() * alpha_diff
            * alpha_bar * (1.0 - alpha_bar) * 4.0
        )

        # 总通量
        total_flux = conv_flux + drift_flux[:n_internal] + compressive

        # 散度
        div = torch.zeros(n_cells, dtype=alpha.dtype, device=alpha.device)
        div = div + scatter_add(total_flux, int_owner, n_cells)
        div = div + scatter_add(-total_flux, int_neigh, n_cells)

        # 时间积分（显式 Euler）
        V_safe = cell_volumes.clamp(min=1e-30)
        alpha_new = alpha - self.delta_t * div / V_safe
    ...
    def _momentum_predictor(
        ...
        # 重力源项
        F_gravity = rho.unsqueeze(-1) * self.g.unsqueeze(0)

        source = H - grad_p + F_gravity
