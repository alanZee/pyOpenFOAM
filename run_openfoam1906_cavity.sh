...
cd $CASE_DIR && /tmp/openfoam1906/usr/bin/blockMesh 2>&1 | tail -5

# Run icoFoam
echo "=== Running icoFoam ==="
cd $CASE_DIR && /tmp/openfoam1906/usr/bin/icoFoam 2>&1 | tail -10

# Extract results
echo "=== Results ==="
if [ -f "$CASE_DIR/0.5/U" ]; then
    echo "U field at t=0.5:"
    grep -A5 'internalField' $CASE_DIR/0.5/U | head -5
fi
