#!/bin/bash
# Run cavity cases with OpenFOAM v12
...
for NX in 8 16 32; do
    CASE_DIR=/tmp/cavity_v12_${NX}x${NX}
    rm -rf $CASE_DIR
...
if match:
    ...
    if ux_values:
        print(f'  ${NX}x${NX}: Ux_max={max(ux_values):.6f} Ux_min={min(ux_values):.6f}')
    else:
        print(f'  ${NX}x${NX}: no values parsed')
else:
    print(f'  ${NX}x${NX}: no nonuniform field found')
"
done
