"""Tests for v12 enhanced turbulent dissipation inlet boundary condition."""
...
class TestTurbulentDissipationInlet12BC:
    """Test the turbulentDissipationInlet12 boundary condition."""

    def test_registration(self):
        assert "turbulentDissipationInlet12" in BoundaryCondition.available_types()

    def test_factory_creation(self, simple_patch):
        bc = BoundaryCondition.create(
            "turbulentDissipationInlet12", simple_patch,
            {"mixingLength": 0.01},
        )
        assert isinstance(bc, TurbulentDissipationInlet12BC)

    def test_type_name(self, simple_patch):
        bc = TurbulentDissipationInlet12BC(simple_patch)
        assert bc.type_name == "turbulentDissipationInlet12"
    ...
    def test_custom_properties(self, simple_patch):
        bc = TurbulentDissipationInlet12BC(simple_patch, {
            "compCoeff": 0.2, "MaLimit": 0.3, "mixingLength": 0.05,
        })
    ...
    def test_apply_with_compressibility(self, simple_patch):
        bc = TurbulentDissipationInlet12BC(simple_patch, {"compCoeff": 0.2})
        k = torch.tensor([1.0, 1.0, 1.0], dtype=torch.float64)
        field = torch.zeros(15, dtype=torch.float64)
    ...
    def test_apply_fallback_without_nu(self, simple_patch):
        bc = TurbulentDissipationInlet12BC(simple_patch, {"mixingLength": 0.02})
        k = torch.tensor([1.0, 1.0, 1.0], dtype=torch.float64)
        field = torch.zeros(15, dtype=torch.float64)
        bc.apply(field, k=k)
    ...
    def test_apply_with_strain_rate(self, simple_patch):
        bc = TurbulentDissipationInlet12BC(simple_patch, {"wallDist": 0.01})
        k = torch.tensor([1.0, 1.0, 1.0], dtype=torch.float64)
