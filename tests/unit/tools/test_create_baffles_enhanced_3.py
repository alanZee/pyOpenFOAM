"""Tests for create_baffles_enhanced_3 — enhanced baffle creation v3."""
...
class TestCreateBafflesEnhanced3:
    def test_returns_result_type(self):
        m = _two_cell_mesh()
        r = create_baffles_enhanced_3(m, face_indices=[0])
        assert isinstance(r, BaffleEnhanced3Result)
    ...
    def test_dual_patches(self):
        m = _two_cell_mesh()
        r = create_baffles_enhanced_3(m, face_indices=[0], dual_patches=True)
        assert len(r.baffle_patches) == 2

    def test_zone_face_counts(self):
        m = _two_cell_mesh()
        r = create_baffles_enhanced_3(m, face_indices=[0], patch_name="test")
        assert "test" in r.zone_face_counts
        assert r.zone_face_counts["test"] == 1

    def test_total_baffle_area_positive(self):
        m = _two_cell_mesh()
        r = create_baffles_enhanced_3(m, face_indices=[0])
        assert r.total_baffle_area > 0

    def test_no_indices_raises(self):
        m = _two_cell_mesh()
        with pytest.raises(ValueError, match="One of"):
            create_baffles_enhanced_3(m)

    def test_min_area_filter(self):
        m = _two_cell_mesh()
        r = create_baffles_enhanced_3(m, face_indices=[0], min_area=1e6)
