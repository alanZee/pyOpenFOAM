"""Tests for merge_meshes_enhanced — enhanced mesh merging."""
...
class TestSingleMesh:
    def test_returns_result_type(self):
        m = _single_hex()
        r = merge_meshes_enhanced([m])
        assert isinstance(r, MergeEnhancedResult)
    ...
    def test_zero_merged_points(self):
        m = _single_hex()
        r = merge_meshes_enhanced([m])
        assert r.n_merged_points == 0
...
class TestRelativeTolerance:
    def test_relative_tolerance(self):
        m1 = _single_hex((0, 0, 0))
        m2 = _single_hex((10, 0, 0))
        r = merge_meshes_enhanced([m1, m2], relative_tolerance=1e-3)
        assert r.mesh.n_cells == 2
...
class TestZoneMerging:
    def test_merge_zones_combines_patches(self):
        m1 = _single_hex((0, 0, 0))
        m2 = _single_hex((10, 0, 0))
        r = merge_meshes_enhanced([m1, m2], merge_zones=True)
        # Both meshes have "all" patch; should merge into one
...
class TestEdgeCases:
    def test_empty_list_raises(self):
        with pytest.raises(ValueError, match="empty"):
            merge_meshes_enhanced([])

    def test_does_not_modify_original(self):
        m = _single_hex()
        nc = m.n_cells
        merge_meshes_enhanced([m])
        assert m.n_cells == nc
