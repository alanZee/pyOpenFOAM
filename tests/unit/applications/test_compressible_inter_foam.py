"""
Test compressibleInterFoam solver.
...
def _make_dam_break_case(
    ...
    """Write a complete compressible dam break case.

    Reuses the same mesh / field structure as the interFoam test but adds a
    temperature field ``T`` required by compressibleInterFoam.
    """
...
def dam_break_case(tmp_path):
    ...
    case_dir = tmp_path / "compressibleDamBreak"
...
class TestCompressibleInterFoam:
    """Tests for the compressibleInterFoam solver."""
    ...
    def test_case_loads(self, dam_break_case):
        ...
        solver = CompressibleInterFoam(dam_break_case)
    ...
    def test_mixture_properties(self, dam_break_case):
        ...
        # At alpha=1 (water cells): rho should be closer to rho2
        # At alpha=0 (air cells): rho should be closer to rho1
        assert rho.max() >= solver.rho1 * 0.9
        assert rho.min() <= solver.rho2 * 1.1
    ...
    def test_piso_loop_executes(self, dam_break_case):
        ...
        # Run one time step
        U, p, alpha, phi, T, conv = solver._pimple_vof_iteration()
    ...
    def test_temperature_computed(self, dam_break_case):
        """Temperature field is computed from EOS after iteration."""
        from pyfoam.applications.compressible_inter_foam import CompressibleInterFoam
        ...
        # (T = p / (rho * Cv_mix), so T=0 when p=0 which is valid)
        assert torch.isfinite(solver.T).all(), "T contains NaN/Inf"
