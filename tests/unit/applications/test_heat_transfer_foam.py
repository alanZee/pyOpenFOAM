"""
Unit tests for HeatTransferFoam — enhanced heat transfer solver.

Tests cover:
- Solver initialisation with default properties
- Custom material property injection (kappa, Cp, rho)
- Radiation model integration
- Pure conduction case (1D wall)
...
def _make_heat_transfer_case(
    ...
    """Write a complete heat transfer case to *case_dir*."""
...
@pytest.fixture
def heat_case(tmp_path):
    """Create a heat transfer case (2x2 mesh for fast tests)."""
    case_dir = tmp_path / "heat_case"
    _make_heat_transfer_case(
        case_dir,
        n_cells_x=2,
        n_cells_y=2,
        T_init=300.0,
...
@pytest.fixture
def heat_case_4x4(tmp_path):
    """Create a 4x4 heat transfer case."""
    case_dir = tmp_path / "heat_4x4"
    _make_heat_transfer_case(
        case_dir,
        n_cells_x=4,
        n_cells_y=4,
...
class TestHeatTransferFoamInit:
    """Tests for HeatTransferFoam initialisation."""

    def test_case_loads(self, heat_case):
    ...
    def test_custom_properties_injection(self, heat_case):
    ...
    def test_temperature_field_shape(self, heat_case):
        """Temperature field has correct shape."""
        from pyfoam.applications.heat_transfer_foam import HeatTransferFoam
        solver = HeatTransferFoam(heat_case)
        assert solver.T.shape == (4,)

    def test_radiation_model_default(self, heat_case):
    ...
    def test_custom_radiation_injection(self, heat_case):
        """Custom radiation model can be injected."""
        from pyfoam.applications.heat_transfer_foam import HeatTransferFoam
        from pyfoam.models.radiation import P1Radiation
        custom_rad = P1Radiation(absorption_coeff=0.5)
        solver = HeatTransferFoam(heat_case, radiation=custom_rad)
        assert solver.radiation is custom_rad
        assert solver.radiation.absorption_coeff == 0.5

    def test_convective_source_params(self, heat_case):
        """Convective heat transfer parameters are stored."""
        from pyfoam.applications.heat_transfer_foam import HeatTransferFoam
        solver = HeatTransferFoam(heat_case, h_conv=10.0, T_inf=350.0)
        assert solver.h_conv == 10.0
        assert solver.T_inf == 350.0


class TestHeatTransferFoamRun:
    """Tests for solver execution."""

    def test_run_produces_finite_values(self, heat_case):
    ...
    def test_run_converges(self, heat_case):
    ...
    def test_pure_conduction_temperature_gradient(self, heat_case_4x4):
        """1D conduction produces a temperature gradient from hot to cold."""
        from pyfoam.applications.heat_transfer_foam import HeatTransferFoam
        solver = HeatTransferFoam(heat_case_4x4, alpha_T=1.0)
        solver.run()

        mesh = solver.mesh
        x = mesh.cell_centres[:, 0]

        # Cells near left (x small) should be hotter than cells near right
        left_mask = x < 0.25
        right_mask = x > 0.75
    ...
    def test_convection_cooling(self, heat_case):
        """With convective source, temperature moves toward T_inf."""
        from pyfoam.applications.heat_transfer_foam import HeatTransferFoam
        solver = HeatTransferFoam(
            heat_case,
            h_conv=100.0,
    ...
    def test_radiation_affects_temperature(self, heat_case):
        """Non-zero radiation changes temperature compared to no radiation."""
        from pyfoam.applications.heat_transfer_foam import HeatTransferFoam
        from pyfoam.models.radiation import P1Radiation

        solver_no_rad = HeatTransferFoam(heat_case, radiation=P1Radiation(absorption_coeff=0.0))
        solver_no_rad.run()

        solver_rad = HeatTransferFoam(
            heat_case,
            radiation=P1Radiation(absorption_coeff=1.0, T_ref=250.0),
        )
        solver_rad.run()

        # With radiation toward T_ref=250 (which is below the T range),
        # temperatures should be shifted
        assert torch.isfinite(solver_rad.T).all()

    def test_under_relaxation_effect(self, heat_case):
        """Lower under-relaxation takes more iterations but still converges."""
        from pyfoam.applications.heat_transfer_foam import HeatTransferFoam
        solver = HeatTransferFoam(heat_case, alpha_T=0.3)
        conv = solver.run()
        assert torch.isfinite(solver.T).all()

    def test_run_writes_output(self, heat_case):
