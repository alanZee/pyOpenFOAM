"""
Tests for v2 ODE solvers: RKCK45_v2, RKDP45_v2, Rosenbrock12_v2,
Rosenbrock23_v2, Rosenbrock34_v2, SIS_v2, SEulex_v2.
...
class TestV2RTSRegistry:
    """Test that v2 solvers are registered in the RTS."""

    @pytest.mark.parametrize(
        "name",
        [
            "RKCK45_v2", "RKDP45_v2", "Rosenbrock12_v2",
            "Rosenbrock23_v2", "Rosenbrock34_v2", "SIS_v2", "SEulex_v2",
        ],
    )
    def test_create_by_name(self, name):
        """All v2 solvers should be creatable by name."""
        solver = create_ode_solver(name)
        assert isinstance(solver, ODESolver)

    def test_registry_contains_all_v2(self):
        """Registry should contain all v2 solver names."""
        create_ode_solver("RKCK45_v2")
        expected = {
            "RKCK45_v2", "RKDP45_v2", "Rosenbrock12_v2",
            "Rosenbrock23_v2", "Rosenbrock34_v2", "SIS_v2", "SEulex_v2",
        }
    ...
    def test_repr_all_v2(self):
        """repr should work for all v2 solvers."""
        for name in [
            "RKCK45_v2", "RKDP45_v2", "Rosenbrock12_v2",
            "Rosenbrock23_v2", "Rosenbrock34_v2", "SIS_v2", "SEulex_v2",
        ]:
...
class TestV2SimpleDecay:
    """Test v2 solvers on dy/dt = -y, y(0) = 1."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("RKCK45_v2", 1e-2, 1e-6),
            ("RKDP45_v2", 1e-2, 1e-6),
            ("Rosenbrock12_v2", 1e-2, 1e-4),
            ("Rosenbrock23_v2", 1e-2, 1e-5),
            ("Rosenbrock34_v2", 1e-2, 1e-6),
            ("SIS_v2", 1e-2, 1e-4),
            ("SEulex_v2", 1e-2, 1e-6),
        ],
    )
...
class TestV2HarmonicOscillator:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("RKCK45_v2", 0.01, 1e-4),
            ("RKDP45_v2", 0.01, 1e-4),
            ("Rosenbrock12_v2", 0.01, 1e-3),
            ("Rosenbrock23_v2", 0.01, 1e-3),
            ("Rosenbrock34_v2", 0.01, 1e-4),
            ("SIS_v2", 0.01, 1e-3),
            ("SEulex_v2", 0.01, 1e-4),
        ],
    )
...
class TestV2AdaptiveSolvers:
    """Test adaptive step-size control for RKCK45_v2 and RKDP45_v2."""

    @pytest.mark.parametrize("solver_cls", [RKCK45Solver_v2, RKDP45Solver_v2])
...
class TestV2StiffODE:
    """Test stiff ODE handling for v2 implicit solvers."""

    @pytest.mark.parametrize(
        "solver_name",
        ["Rosenbrock12_v2", "Rosenbrock23_v2", "Rosenbrock34_v2", "SIS_v2", "SEulex_v2"],
    )
    def test_implicit_handle_stiffness(self, solver_name):
        """Implicit/semi-implicit v2 solvers should handle stiff problems."""
        lam = 1000.0
...
class TestV2ConvergenceOrder:
    """Verify convergence order for v2 explicit solvers."""
    ...
    @pytest.mark.parametrize("solver_name", ["RKCK45_v2", "RKDP45_v2"])
    def test_fourth_order_convergence(self, solver_name):
        """RKCK45_v2 and RKDP45_v2 should converge at ~O(dt^4) or better."""
        f = lambda t, y: -y
...
class TestV2EdgeCases:
    """Edge case tests for v2 solvers."""
    ...
    def test_custom_tolerances(self):
        ...
        solver = create_ode_solver("RKCK45_v2", rtol=1e-10, atol=1e-12)
        assert solver.rtol == 1e-10
        assert solver.atol == 1e-12

        solver = create_ode_solver("RKDP45_v2", rtol=1e-8, atol=1e-10)
        assert solver.rtol == 1e-8
        assert solver.atol == 1e-10

    def test_adaptive_custom_params(self):
        """Adaptive v2 solvers should accept custom min_scale/max_scale."""
        solver = RKCK45Solver_v2(min_scale=0.1, max_scale=10.0, safety=0.8)
        assert solver._min_scale == 0.1
        assert solver._max_scale == 10.0
        assert solver._safety == 0.8

        solver = RKDP45Solver_v2(min_scale=0.1, max_scale=10.0, safety=0.8)
        assert solver._min_scale == 0.1
