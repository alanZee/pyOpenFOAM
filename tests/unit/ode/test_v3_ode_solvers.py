"""
Tests for v3 ODE solvers: RKCK45_v3, RKDP45_v3, Rosenbrock12_v3,
Rosenbrock23_v3, Rosenbrock34_v3, SIS_v3, SEulex_v3.
...
class TestV3RTSRegistry:
    """Test that v3 solvers are registered in the RTS."""

    @pytest.mark.parametrize(
        "name",
        [
            "RKCK45_v3", "RKDP45_v3", "Rosenbrock12_v3",
            "Rosenbrock23_v3", "Rosenbrock34_v3", "SIS_v3", "SEulex_v3",
        ],
    )
    def test_create_by_name(self, name):
        """All v3 solvers should be creatable by name."""
        solver = create_ode_solver(name)
        assert isinstance(solver, ODESolver)

    def test_registry_contains_all_v3(self):
        """Registry should contain all v3 solver names."""
        create_ode_solver("RKCK45_v3")
        expected = {
            "RKCK45_v3", "RKDP45_v3", "Rosenbrock12_v3",
            "Rosenbrock23_v3", "Rosenbrock34_v3", "SIS_v3", "SEulex_v3",
        }
    ...
    def test_repr_all_v3(self):
        """repr should work for all v3 solvers."""
        for name in [
            "RKCK45_v3", "RKDP45_v3", "Rosenbrock12_v3",
            "Rosenbrock23_v3", "Rosenbrock34_v3", "SIS_v3", "SEulex_v3",
        ]:
...
class TestV3SimpleDecay:
    """Test v3 solvers on dy/dt = -y, y(0) = 1."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("RKCK45_v3", 1e-2, 1e-6),
            ("RKDP45_v3", 1e-2, 1e-6),
            ("Rosenbrock12_v3", 1e-2, 1e-4),
            ("Rosenbrock23_v3", 1e-2, 1e-5),
            ("Rosenbrock34_v3", 1e-2, 1e-6),
            ("SIS_v3", 1e-2, 1e-4),
            ("SEulex_v3", 1e-2, 1e-6),
        ],
    )
...
class TestV3HarmonicOscillator:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("RKCK45_v3", 0.01, 1e-4),
            ("RKDP45_v3", 0.01, 1e-4),
            ("Rosenbrock12_v3", 0.01, 1e-3),
            ("Rosenbrock23_v3", 0.01, 1e-3),
            ("Rosenbrock34_v3", 0.01, 1e-4),
            ("SIS_v3", 0.01, 1e-3),
            ("SEulex_v3", 0.01, 1e-4),
        ],
    )
...
class TestV3AdaptiveSolvers:
    """Test adaptive step-size control for RKCK45_v3 and RKDP45_v3."""

    @pytest.mark.parametrize("solver_cls", [RKCK45Solver_v3, RKDP45Solver_v3])
...
class TestV3StiffODE:
    """Test stiff ODE handling for v3 implicit solvers."""

    @pytest.mark.parametrize(
        "solver_name",
        ["Rosenbrock12_v3", "Rosenbrock23_v3", "Rosenbrock34_v3", "SIS_v3", "SEulex_v3"],
    )
    def test_implicit_handle_stiffness(self, solver_name):
        """Implicit/semi-implicit v3 solvers should handle stiff problems."""
        lam = 1000.0
...
class TestV3EdgeCases:
    """Edge case tests for v3 solvers."""
    ...
    def test_custom_tolerances(self):
        ...
        solver = create_ode_solver("RKCK45_v3", rtol=1e-10, atol=1e-12)
        assert solver.rtol == 1e-10
        assert solver.atol == 1e-12

        solver = create_ode_solver("RKDP45_v3", rtol=1e-8, atol=1e-10)
        assert solver.rtol == 1e-8
        assert solver.atol == 1e-10

    def test_adaptive_custom_params(self):
        """Adaptive v3 solvers should accept custom parameters."""
        solver = RKCK45Solver_v3(min_scale=0.1, max_scale=10.0, safety=0.8)
        assert solver._min_scale == 0.1
        assert solver._max_scale == 10.0
        assert solver._safety == 0.8

        solver = RKDP45Solver_v3(min_scale=0.1, max_scale=10.0, safety=0.8)
        assert solver._min_scale == 0.1

    def test_pid_controller_resets(self, simple_decay):
        """PID controller history should reset between integrations."""
        f, y0, exact = simple_decay
        solver = create_ode_solver("RKCK45_v3")

        # First integration
        times1, states1 = solver.integrate(f, (0.0, 1.0), y0, dt=0.1)
        # Second integration — PID history should not affect result
        times2, states2 = solver.integrate(f, (0.0, 1.0), y0, dt=0.1)
        assert torch.allclose(states1[-1], states2[-1], atol=1e-10)

    def test_pi_controller_resets(self, simple_decay):
        """PI controller history should reset between integrations."""
        f, y0, exact = simple_decay
        solver = create_ode_solver("RKDP45_v3")

        # First integration
        times1, states1 = solver.integrate(f, (0.0, 1.0), y0, dt=0.1)
        # Second integration — PI history should not affect result
        times2, states2 = solver.integrate(f, (0.0, 1.0), y0, dt=0.1)
        assert torch.allclose(states1[-1], states2[-1], atol=1e-10)
