"""
Tutorial validation: solver wall shear stress tests.
...
def _make_shear_case(tmp_path: Path, name: str, nx: int = 10, ny: int = 10) -> Path:
    case = tmp_path / name
...
class TestWallShearStress:
    ...
    def test_wall_shear_stress_finite(self, tmp_path: Path):
        """壁面剪切应力有限。"""
        case = _make_shear_case(tmp_path, "shear")
...
class TestWallShearEffect:
    """壁面剪切效应测试。"""

    def test_shear_affects_flow(self, tmp_path: Path):
        """剪切影响流场。"""
        case = _make_shear_case(tmp_path, "shear_effect")
        from pyfoam.applications.simple_foam import SimpleFoam
        solver = SimpleFoam(case)
        solver.run()
        # 检查流场有梯度
