"""
Tutorial validation: solver boundary condition effect tests.
...
class TestBoundaryConditionEffect:
    ...
    def test_wall_velocity_produces_flow(self, tmp_path: Path):
        """壁面速度产生流场。"""
        case = _make_bc_effect_case(tmp_path, "wall_flow", u_wall=1.0)
    ...
    def test_higher_wall_velocity(self, tmp_path: Path):
        """更高壁面速度产生更强流场。"""
        case1 = _make_bc_effect_case(tmp_path, "v1", u_wall=1.0)
        case2 = _make_bc_effect_case(tmp_path, "v2", u_wall=10.0)
        from pyfoam.applications.simple_foam import SimpleFoam
    ...
    def test_lower_viscosity(self, tmp_path: Path):
        """更低粘度产生更强流场。"""
        case1 = _make_bc_effect_case(tmp_path, "nu1", nu=0.1)
        case2 = _make_bc_effect_case(tmp_path, "nu2", nu=0.001)
        from pyfoam.applications.simple_foam import SimpleFoam
        solver1 = SimpleFoam(case1)
        solver1.run()
        solver2 = SimpleFoam(case2)
...
class TestBoundaryConditionSymmetry:
    """边界条件对称性测试。"""

    def test_symmetric_geometry(self, tmp_path: Path):
        """对称几何产生对称流场。"""
        case = _make_bc_effect_case(tmp_path, "sym", nx=10, ny=10)
        from pyfoam.applications.simple_foam import SimpleFoam
        solver = SimpleFoam(case)
        solver.run()
        # 检查流场有限
