"""
Tutorial validation: solver temperature field tests.

验证求解器温度场处理。
"""
from __future__ import annotations

from pathlib import Path

import pytest
import torch

from pyfoam.core.dtype import CFD_DTYPE, INDEX_DTYPE
from tests.tutorials.helpers import (
    make_structured_mesh,
    write_control_dict,
    write_fv_schemes,
    write_fv_solution,
    write_pressure_field,
    write_transport_properties,
    write_velocity_field,
)


def _make_temp_case(tmp_path: Path, name: str, nx: int = 10, ny: int = 10) -> Path:
    case = tmp_path / name
    mesh_dir = case / "constant" / "polyMesh"
    make_structured_mesh(mesh_dir, nx=nx, ny=ny)
    write_transport_properties(case, nu=0.01)
    write_control_dict(case, delta_t=0.005, end_time=1.0, write_interval=200)
    write_fv_schemes(case)
    write_fv_solution(case)
    write_velocity_field(
        case,
        patches={"movingWall": (1, 0, 0), "fixedWalls": (0, 0, 0), "frontAndBack": (0, 0, 0)},
        bc_types={"movingWall": "fixedValue", "fixedWalls": "noSlip", "frontAndBack": "empty"},
    )
    write_pressure_field(
        case,
        patches={"movingWall": "zeroGradient", "fixedWalls": "zeroGradient", "frontAndBack": "empty"},
    )
    return case


class TestTemperatureField:
    """温度场测试。"""

    def test_laplacian_foam_import(self):
        """LaplacianFoam 可导入。"""
        from pyfoam.applications import LaplacianFoam
        assert LaplacianFoam is not None

    def test_heat_transfer_foam_import(self):
        """HeatTransferFoam 可导入。"""
        from pyfoam.applications import HeatTransferFoam
        assert HeatTransferFoam is not None

    def test_buoyant_simple_foam_import(self):
        """BuoyantSimpleFoam 可导入。"""
        from pyfoam.applications import BuoyantSimpleFoam
        assert BuoyantSimpleFoam is not None

    def test_buoyant_pimple_foam_import(self):
        """BuoyantPimpleFoam 可导入。"""
        from pyfoam.applications import BuoyantPimpleFoam
        assert BuoyantPimpleFoam is not None


class TestTemperatureSolver:
    """温度求解器测试。"""

    @pytest.mark.xfail(reason="LaplacianFoam 需要 T 场文件")
    def test_laplacian_foam_runs(self, tmp_path: Path):
        """LaplacianFoam 运行。"""
        case = _make_temp_case(tmp_path, "laplacian")
        from pyfoam.applications.laplacian_foam import LaplacianFoam
        solver = LaplacianFoam(case)
        solver.run()
        assert torch.isfinite(solver.U).all()
