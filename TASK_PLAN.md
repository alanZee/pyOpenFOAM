...
|---------|------|------|--------|------|----------|
| **T0.1** | **修复 FvMatrix 矩阵-向量乘积量纲** | 无 | 🔴 CRITICAL | ❌ 待做 | 矩阵系数和源项量纲一致 |
| T0.1.1 | 分析 OpenFOAM FvMatrix::operator& 实现 | 无 | 🔴 | ❌ | 理解 1/V 应用位置 |
| T0.1.2 | 修改 LduMatrix.Ax() 应用 1/V 因子 | T0.1.1 | 🔴 | ❌ | 矩阵-向量乘积包含 1/V |
| T0.1.3 | 或修改 assemble_pressure_equation() 源项÷V | T0.1.1 | 🔴 | ❌ | 源项与矩阵量纲一致 |
| T0.1.4 | 调整松弛因子以匹配新量级 | T0.1.2 或 T0.1.3 | 🔴 | ❌ | 求解器稳定收敛 |
| **T0.2** | **验证 Ghia 基准案例** | T0.1 | 🔴 CRITICAL | ❌ 待做 | u ≈ -0.20581 在 y=0.5 |
| T0.2.1 | 运行 lid_driven_cavity.py 验证 | T0.1 | 🔴 | ❌ | L2 误差 < 5% |
| T0.2.2 | 对比 Ghia 数据绘图 | T0.2.1 | 🔴 | ❌ | 速度剖面匹配 |
