#!/bin/bash
# Run Couette flow reference with OpenFOAM v12
...
# Couette flow: 2D channel, top wall moves at U=1, bottom stationary
# Analytical: U(y) = U_wall * y/H = y, U_max = 1.0 at y=H
echo "=== Couette Flow Reference (OpenFOAM v12) ==="
for NX in 8 16 32; do
    CASE_DIR=/tmp/couette_v12_${NX}x${NX}
    rm -rf $CASE_DIR
...
blocks (hex (0 1 2 3 4 5 6 7) (1 $NX 1) simpleGrading (1 1 1));
edges ();
boundary (
...
if m:
    ...
    if ux:
        print(f'  ${NX}x1: Ux_max={max(ux):.6f} Ux_min={min(ux):.6f}')
        # Analytical: U(y) = U_wall * y/H, so U_max should be ~1.0
        print(f'  Analytical U_max = 1.0')
"
done
